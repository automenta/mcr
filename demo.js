...
class Example {
  ...
  async createSession() {
    ...
    try {
      ...
      if (this.strategyToUse) {
        // strategyToUse will be set on the instance
      ...
      if (createResponse.data.activeStrategy) {
        this.dLog.info(
          'Session confirmed with active strategy',
          createResponse.data.activeStrategy
        );
      }
  ...
  async assertFact(fact, type = 'text') {
    // Added type parameter, defaults to 'text'
    ...
    const factSnippet = isProlog
      ? fact.substring(0, 200) + (fact.length > 200 ? '...' : '')
      : fact;
  ...
  async query(question) {
    ...
    try {
      ...
      if (queryResponse.data.debugInfo) {
        this.logger.debug(
          // Keep detailed debug log for file
          `[${this.getName()}] Query Debug Info:`,
          queryResponse.data.debugInfo
        );
        if (queryResponse.data.debugInfo.prologQuery) {
          this.dLog.logic(
            'Prolog Query Sent to KB',
            queryResponse.data.debugInfo.prologQuery
          );
        }
        if (
          queryResponse.data.debugInfo.kbResults &&
          queryResponse.data.debugInfo.kbResults.length > 0
        ) {
          this.dLog.logic(
            'Raw KB Results',
            queryResponse.data.debugInfo.kbResults.join('; ')
          );
        } else if (queryResponse.data.debugInfo.kbResults) {
          // if kbResults exists but might be empty
  ...
  handleApiError(error, message) {
    ...
    if (error.response) {
      this.dLog.error(`API Error Status`, error.response.status);
      this.dLog.error(`API Error Data`, error.response.data);
  ...
  async assertCondition(condition, successMessage, failureMessage) {
    if (condition) {
      ...
      this.logger.error(
        `[${this.getName()}] Assertion FAILED: ${failureMessage}`
      ); // Keep error in main log
    }
  }
  ...
  async printKnowledgeBase() {
    ...
    try {
      ...
      const response = await axios.get(
        `${this.apiBaseUrl}/sessions/${this.sessionId}`
      );
  ...
  files.forEach((file) => {
    if (file.endsWith('Demo.js') && file !== 'demoUtils.js') {
      ...
      try {
        ...
        if (
          typeof LoadedClass === 'function' &&
          LoadedClass.prototype instanceof Example
        ) {
          ExampleClassToUse = LoadedClass;
        } else if (
          typeof LoadedClass.default === 'function' &&
          LoadedClass.default.prototype instanceof Example
        ) {
        ...
        if (ExampleClassToUse) {
          ...
          const tempInstance = new ExampleClassToUse(
            API_BASE_URL,
            logger,
            demoLogger
          );
          const key = tempInstance.getName().toLowerCase().replace(/\s+/g, '-');
          exampleBlueprints[key] = {
            Class: ExampleClassToUse,
            defaultInstance: tempInstance,
          };
        }
      } catch (err) {
        console.error(
          chalk.red(
            `Error loading demo blueprint ${exampleName}: ${err.message}`
          )
        );
        logger.error(
          `Failed to load demo blueprint ${exampleName}: ${err.stack}`
        );
      }
    }
  });
...
async function main() {
  ...
  const argv = yargs(hideBin(process.argv))
    .command('$0 [exampleName]', 'Run a specific MCR demo example', (y) => {
      y.positional('exampleName', {
        ...
        type: 'string',
        choices:
          Object.keys(exampleBlueprints).length > 0
            ? Object.keys(exampleBlueprints)
            : undefined,
      });
    })
    .option('list', {
    ...
    .option('strategy', {
      ...
      type: 'string',
      description:
        'Specify the translation strategy to use (e.g., SIR-R1) or "all" to run with all available strategies. Server default is used if not specified.',
      default: null,
    })
    .help()
    .alias('help', 'h')
  ...
  if (
    argv.list ||
    (!argv.exampleName && Object.keys(exampleBlueprints).length > 0)
  ) {
  ...
  if (
    ...
    !KNOWN_STRATEGIES.includes(config.translationStrategy)
  ) {
  ...
  if (argv.strategy) {
    if (argv.strategy.toLowerCase() === 'all') {
      ...
      if (strategiesToRun.length === 0) {
        demoLogger.warn(
          '"--strategy all" was specified, but no strategies are hardcoded/known. Using server default.'
        );
      ...
      if (!KNOWN_STRATEGIES.includes(argv.strategy)) {
        demoLogger.warn(
          `Strategy "${argv.strategy}" is not in the client-side known list (${KNOWN_STRATEGIES.join(', ')}). Attempting to use it anyway.`
        );
      }
    }
  } else {
  ...
  for (let i = 0; i < strategiesToRun.length; i++) {
    ...
    const strategyLabel = strategyName
      ? `Strategy: ${strategyName}`
      : 'Server Default Strategy';

    demoLogger.heading(
      `Running Demo: ${exampleToRun.getName()} (${runId} - ${strategyLabel})`
    );
    ...
    try {
      ...
      demoLogger.error(
        `Critical error during demo "${exampleToRun.getName()}" with ${strategyLabel}`,
        error.message
      );
      logger.error(
      ...
      demoLogger.divider();
      demoLogger.heading(
        `Demo ${exampleToRun.getName()} (${strategyLabel}) Finished`
      );
