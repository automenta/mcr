 ...
 */
async function generateContent({ promptName, systemPrompt, userPrompt, llmProviderName, modelName }) {
  logger.info(
  ...
  try {
    if (currentProvider) {
      ...
      if (currentModel) {
        ...
        if (currentProvider.toLowerCase() === 'openai') process.env.MCR_LLM_MODEL_OPENAI = currentModel;
        else if (currentProvider.toLowerCase() === 'gemini') process.env.MCR_LLM_MODEL_GEMINI = currentModel;
        else if (currentProvider.toLowerCase() === 'ollama') process.env.MCR_LLM_MODEL_OLLAMA = currentModel;
        else logger.warn(`[GeneratorUtils] Model specified for unknown provider ${currentProvider}, model override might not take effect as expected.`);
      }
    ...
    logger.info(`[GeneratorUtils] Calling LLM service. Provider: ${process.env.MCR_LLM_PROVIDER}, Model for OpenAI: ${process.env.MCR_LLM_MODEL_OPENAI}, Model for Gemini: ${process.env.MCR_LLM_MODEL_GEMINI}, Model for Ollama: ${process.env.MCR_LLM_MODEL_OLLAMA}`);

    const llmResult = await llmService.generate(systemPrompt, userPrompt);

    if (!llmResult || typeof llmResult.text !== 'string') {
        logger.error(`[GeneratorUtils] LLM generation for "${promptName}" returned invalid or empty result.`, { llmResult });
        throw new Error(`LLM generation for "${promptName}" returned invalid or empty result.`);
    }
    logger.debug(`[GeneratorUtils] LLM Raw Output for ${promptName}:\n${llmResult.text}`);
    return llmResult.text;

  } catch (error) {
    logger.error(`[GeneratorUtils] Error during LLM generation for "${promptName}": ${error.message}`, error);
    throw error; // Re-throw to be handled by the calling script
  } finally {
    // Restore original environment variables
    if (llmProviderName) { // Only restore if we actually changed them
      ...
      logger.info(`[GeneratorUtils] Restored original LLM environment variables.`);
 ...
 */
function setupGeneratorScript(scriptSpecificOptions, scriptFileName = 'generator_script.js') {
  let yargsInstance = yargs(hideBin(process.argv));

  // Add script-specific options
  for (const optionName in scriptSpecificOptions) {
    yargsInstance = yargsInstance.option(optionName, scriptSpecificOptions[optionName]);
  }

  // Add common options, applying default for provider dynamically
  yargsInstance = yargsInstance.option('provider', {
  ...
  return yargsInstance.help().alias('help', 'h')
    .epilogue(`Run with ${scriptFileName} --help for more information.`)
    .argv;
}
...
function ensureDirectoryExists(dirPath) {
  if (!fs.existsSync(dirPath)) {
    logger.info(`[GeneratorUtils] Directory ${dirPath} does not exist, creating it.`);
 ...
 */
function writeGeneratedFile({ filePath, content, domain, instructions, scriptName }) {
  const header = `// Generated by ${scriptName} for domain: ${domain}\n// Instructions: ${instructions}\n\n`;
  const fileContent = header + content;
  fs.writeFileSync(filePath, fileContent);
  logger.info(`[GeneratorUtils] Successfully generated file and saved to ${filePath}`);
}
