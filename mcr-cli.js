...
program
    ...
    .version('2.0.0')
    .option('-o, --ontology <file>', 'Specify an ontology file to use (e.g., ./ontologies/my_ontology.pl)');

// Helper to handle API errors
const handleApiError = (error) => {
    if (error.response) {
        console.error(`Error: ${error.response.status} - ${error.response.data.error.message || error.response.statusText}`);
...
program.command('get-session <sessionId>')
...
program.command('delete-session <sessionId>')
...
program.command('assert <sessionId> <text>')
    .description('Assert natural language facts into a session')
    .action(async (sessionId, text) => {
        try {
            const response = await axios.post(`${API_BASE_URL}/sessions/${sessionId}/assert`, { text });
...
program.command('query [sessionId] [question]')
    ...
    .option('-o, --ontology <file>', 'Specify an ontology file to use for this query')
    .action(async (sessionIdArg, questionArg, options) => {
        ...
        try {
            ...
            // If no session ID or question is provided, enter interactive mode
...
program.command('explain-query <sessionId> <question>')
    .description('Get an explanation for a natural language query')
    .action(async (sessionId, question) => {
        try {
            const response = await axios.post(`${API_BASE_URL}/sessions/${sessionId}/explain-query`, { query: question });
...
program.command('nl-to-rules <text>')
    ...
    .action(async (text, options) => {
        try {
            const response = await axios.post(`${API_BASE_URL}/translate/nl-to-rules`, {
                text,
                existing_facts: options.existingFacts,
                ontology_context: options.ontologyContext
            });
            console.log('Translated Rules:', JSON.stringify(response.data.rules, null, 2));
        } catch (error) {
            handleApiError(error);
        }
    });

program.command('rules-to-nl <rulesFile>')
    ...
    .action(async (rulesFile, options) => {
        try {
            ...
            const rules = rulesContent.split(/\r?\n|\./).filter(line => line.trim() !== '').map(line => line.trim() + '.'); // Basic parsing
            
            const response = await axios.post(`${API_BASE_URL}/translate/rules-to-nl`, {
                rules,
                style: options.style
            });
...
// --- Ontology Management ---
program.command('add-ontology <name> <rulesFile>')
    ...
    .action(async (name, rulesFile) => {
        try {
            ...
            const response = await axios.post(`${API_BASE_URL}/ontologies`, { name, rules });
...
program.command('update-ontology <name> <rulesFile>')
    ...
    .action(async (name, rulesFile) => {
        try {
            ...
            const rules = fs.readFileSync(rulesPath, 'utf8');
            const response = await axios.put(`${API_BASE_URL}/ontologies/${name}`, { rules });
...
program.command('get-ontologies')
...
program.command('get-ontology <name>')
...
program.command('delete-ontology <name>')
...
program.command('chat')
    ...
    .option('-o, --ontology <file>', 'Specify an ontology file to use for the session')
