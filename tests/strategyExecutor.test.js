...
describe('StrategyExecutor', () => {
  describe('Conditional_Router Node', () => {
    it('should route to target if property_exists condition is met', async () => {
      ...
      mockLlmProvider.generate.mockResolvedValueOnce({ text: JSON.stringify({ message: 'data with error', error: true }) }); // For 'start' node LLM call
      mockLlmProvider.generate.mockResolvedValueOnce({ text: 'error_handled_output' }); // For 'error_handler' LLM call
      ...
      const correctedStrategyJson = {
        ...
        name: 'Test Conditional Route',
        nodes: [
          { id: 'start', type: 'LLM_Call', prompt_text: {user: "test"}, output_variable: 'start_output_raw' },
          { id: 'parse_start_data', type: 'Parse_JSON', input_variable: 'start_output_raw', output_variable: 'data'},
          {
            id: 'router',
            type: 'Conditional_Router',
            input_variable: 'data',
      ...
      const result = await executor.execute(mockLlmProvider, mockReasonerProvider, {});
    ...
    it('should route to default target if no other condition is met', async () => {
      const strategyJson = {
        ...
        nodes: [
          { id: 'start', type: 'LLM_Call', prompt_text: {user: "test"}, output_variable: 'data_raw' },
          { id: 'parse_data', type: 'Parse_JSON', input_variable: 'data_raw', output_variable: 'data'},
          {
            id: 'router',
            type: 'Conditional_Router',
            input_variable: 'data',
      ...
      mockLlmProvider.generate.mockResolvedValueOnce({ text: JSON.stringify({ message: 'data without error' }) }); // For 'start' node
      mockLlmProvider.generate.mockResolvedValueOnce({ text: 'default_handled_output' }); // For 'default_handler' node
    ...
    it('should handle missing input_variable for Conditional_Router', async () => {
        const strategyJson = {
            id: 'test-conditional-missing-var', name: 'Test Missing Var',
            nodes: [
                { id: 'router', type: 'Conditional_Router', branches: [{ condition: 'default', target: 'end' }] },
                { id: 'end', type: 'LLM_Call', prompt_text: {user: "end"}, output_variable: 'final_result'}
            ],
            edges: [], result_variable: 'final_result'
        };
        const executor = new StrategyExecutor(strategyJson);
        try {
            await executor.execute(mockLlmProvider, mockReasonerProvider, {});
        } catch (e) {
            expect(e).toBeInstanceOf(MCRError);
            expect(e.message).toContain("Conditional_Router node router missing 'input_variable'.");
            expect(e.code).toBe(ErrorCodes.INVALID_STRATEGY_NODE); // Check specific code from the original error
        }
    });
  });
  ...
  describe('LF_To_Prolog Node and convertLfToProlog function', () => {
    const baseLfStrategy = {
      ...
      nodes: [
        ...
        { id: 'lf_converter', type: 'LF_To_Prolog', input_variable: 'lf_input', output_variable: 'prolog_output' }
      ],
      edges: [],
    ...
    it('should convert a simple LF fact to Prolog', async () => {
      ...
      const lfFact = { type: 'fact', term: { predicate: 'human', args: [{ type: 'atom', value: 'socrates' }] } };
      const result = await executor.execute(mockLlmProvider, mockReasonerProvider, { lf_input: lfFact });
    ...
    it('should convert an LF rule to Prolog', async () => {
      ...
      const result = await executor.execute(mockLlmProvider, mockReasonerProvider, { lf_input: lfRule });
    ...
    it('should convert an LF rule with multiple body goals', async () => {
        ...
        const lfRule = {
          type: 'rule',
          head: { predicate: 'grandfather', args: [{ type: 'variable', name: 'G' }, { type: 'variable', name: 'C' }] },
          body: [
            { predicate: 'father', args: [{ type: 'variable', name: 'G' }, { type: 'variable', name: 'P' }] },
            { predicate: 'parent', args: [{ type: 'variable', name: 'P' }, { type: 'variable', name: 'C' }] }
          ]
        };
        const result = await executor.execute(mockLlmProvider, mockReasonerProvider, { lf_input: lfRule });
    ...
    it('should handle LF atoms needing quotes', async () => {
        const strategy = JSON.parse(JSON.stringify(baseLfStrategy));
        const executor = new StrategyExecutor(strategy);
        const lfFact = { type: 'fact', term: { predicate: 'has-prop', args: [{ type: 'atom', value: 'object1' }, {type: 'atom', value: 'a value with spaces'}] } };
        const result = await executor.execute(mockLlmProvider, mockReasonerProvider, { lf_input: lfFact });
    ...
    it('should handle lists in LF arguments', async () => {
        ...
        const lfFact = {
            ...
            term: {
                predicate: 'processed',
                args: [
                    { type: 'list', elements: [{ type: 'atom', value: 'a' }, { type: 'number', value: 1 }] }
                ]
            }
        };
        const result = await executor.execute(mockLlmProvider, mockReasonerProvider, { lf_input: lfFact });
        ...
        const lfFact = {
            ...
            term: {
                ...
                args: [
                    ...
                    { type: 'term', value: { predicate: 'pair', args: [{type: 'number', value: 10}, {type: 'atom', value: 'x'}] } }
                ]
            }
        };
        const result = await executor.execute(mockLlmProvider, mockReasonerProvider, { lf_input: lfFact });
    ...
    it('should handle negated terms in LF rule body', async () => {
        ...
        const lfRule = {
          ...
          body: [
            ...
            { predicate: 'penguin', args: [{ type: 'variable', name: 'X' }], isNegative: true }
          ]
        };
        const result = await executor.execute(mockLlmProvider, mockReasonerProvider, { lf_input: lfRule });
    ...
    it('should throw error for invalid LF structure (e.g. missing type in arg)', async () => {
        const strategy = JSON.parse(JSON.stringify(baseLfStrategy));
        const executor = new StrategyExecutor(strategy);
        const lfInvalid = { type: 'fact', term: { predicate: 'test', args: [{ value: 'baddata' }] } }; // Arg missing type
        await expect(executor.execute(mockLlmProvider, mockReasonerProvider, { lf_input: lfInvalid }))
    ...
    it('should process an array of LF items', async () => {
        ...
        const executor = new StrategyExecutor(strategy);
        const lfArray = [
            { type: 'fact', term: { predicate: 'p', args: [{type: 'atom', value: 'a'}] } },
            { type: 'fact', term: { predicate: 'q', args: [{type: 'atom', value: 'b'}] } }
        ];
        const result = await executor.execute(mockLlmProvider, mockReasonerProvider, { lf_input: lfArray });
...
describe('convertSirToProlog (existing function)', () => {
    ...
    const baseSirStrategy = {
        ...
        nodes: [
          { id: 'sir_converter', type: 'SIR_To_Prolog', input_variable: 'sir_input', output_variable: 'prolog_output' }
        ],
        edges: [],
        result_variable: 'prolog_output'
      };
    ...
    it('should convert a simple SIR fact', async () => {
        ...
        const executor = new StrategyExecutor(strategy);
        const sirFact = { statementType: 'fact', fact: { predicate: 'human', arguments: ['socrates'] } };
        const result = await executor.execute(mockLlmProvider, mockReasonerProvider, { sir_input: sirFact });
        expect(result).toEqual(['human(socrates).']);
    });
});
