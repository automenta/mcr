			...
			.mockImplementation((systemPrompt, userPrompt) => {
				...
				if (
					systemPrompt.startsWith(
						'You are an expert AI assistant that translates natural language questions into Prolog queries.'
					) &&
					userPrompt.includes('Question: "What facts are there?"')
				) {
				...
				if (
					systemPrompt.startsWith(
						'You are an expert AI assistant that explains Prolog query results in concise, natural language.'
					) &&
					userPrompt.includes('Prolog Query Results:')
				) {
					return Promise.resolve({
						text: 'The known facts are fact(a) and fact(b).',
					});
				}
