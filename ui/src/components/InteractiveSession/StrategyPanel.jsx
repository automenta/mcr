  ...
  const listStrategies = useCallback(async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        addMessageToHistory({type: 'system', text: `❌ Error listing strategies: ${response.message}`});
        setStrategies([]);
      }
    } catch (error) {
      addMessageToHistory({type: 'system', text: `❌ Error: ${error.message || 'Failed to list strategies'}`});
      setStrategies([]);
    }
  }, [isMcrSessionActive, isWsServiceConnected, addMessageToHistory, setStrategies]);

  useEffect(() => {
    if (isMcrSessionActive && isWsServiceConnected) { // Condition also in listStrategies
      listStrategies();
    } else {
      setStrategies([]);
    }
  ...
  const handleSetStrategy = async (strategyId) => {
    ...
    try {
      ...
      if (response.success && response.data?.activeStrategyId) {
        ...
        addMessageToHistory({type: 'system', text: `❌ Error setting strategy: ${response.message || 'Unknown error'}`});
      }
    } catch (error) {
      addMessageToHistory({type: 'system', text: `❌ Error: ${error.message || 'Failed to set strategy'}`});
    }
  };
