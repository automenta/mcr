  ...
  const listOntologies = useCallback(async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        addMessageToHistory({type: 'system', text: `❌ Error listing ontologies: ${response.message}`});
        setOntologies([]);
      }
    } catch (error) {
      addMessageToHistory({type: 'system', text: `❌ Error: ${error.message || 'Failed to list ontologies'}`});
      setOntologies([]);
    }
  }, [isMcrSessionActive, isWsServiceConnected, addMessageToHistory, setOntologies]);

  useEffect(() => {
    if (isMcrSessionActive && isWsServiceConnected) { // Condition also in listOntologies
      listOntologies();
    } else {
      setOntologies([]);
    }
    ...
    try {
      ...
      if (ontResponse.success && ontResponse.data?.rules) {
        ...
        if (assertResponse.success) {
          addMessageToHistory({type: 'system', text: `✅ Ontology '${ontologyName}' rules asserted successfully. KB will update.`});
