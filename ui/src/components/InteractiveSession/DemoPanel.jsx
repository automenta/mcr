...
const DemoPanel = ({
  ...
  const handleListDemos = useCallback(async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        addMessageToHistory({
          type: 'system',
          text: `Error listing demos: ${response.message || 'Unknown error'}`,
        });
        setDemos([]);
      }
    } catch (error) {
      addMessageToHistory({
        type: 'system',
        text: `Error: ${error.message || 'Failed to list demos'}`,
      });
      setDemos([]);
    }
  ...
  useEffect(() => {
    if (isMcrSessionActive && isWsServiceConnected) {
      // Condition already in handleListDemos
      handleListDemos();
    } else {
  ...
  const handleRunDemo = async demoId => {
    ...
    addMessageToHistory({
      type: 'system',
      text: `🚀 Attempting to run demo: ${demoId}...`,
    });
    try {
      ...
      addMessageToHistory({
        ...
        response: {
          success: false,
          message: `Client-side error running demo ${demoId}: ${error.message}`,
        },
  ...
  return (
    <div>
      <h4>🚀 Demos</h4>
      <button
        onClick={handleListDemos}
        disabled={!isMcrSessionActive || !isWsServiceConnected}
        title="Refresh Demo List"
      >
        🔄 List Demos
      </button>
      {demos.length === 0 && isMcrSessionActive && (
      ...
      <ul>
        {demos.map(demo => (
          <li key={demo.id}>
            ...
            >
              <span>
                {demo.name} <small>({demo.id})</small>
              </span>
              <button
                onClick={() => handleRunDemo(demo.id)}
                disabled={
                  !isMcrSessionActive || !sessionId || !isWsServiceConnected
                }
                title="Run Demo"
              >
                ▶️ Run
