...
class StrategyManager {
   ...
   */
  loadStrategies() {
    logger.info(`[StrategyManager] Loading translation strategies from JSON files in ${STRATEGIES_DIR_JSON}...`);
    this.strategies.clear(); // Clear any previously loaded strategies

    try {
      if (!fs.existsSync(STRATEGIES_DIR_JSON)) {
        logger.warn(`[StrategyManager] Strategies directory not found: ${STRATEGIES_DIR_JSON}. No strategies will be loaded.`);
        return;
      }
        ...
        if (file.endsWith('.json')) {
          ...
          try {
            ...
            if (strategyJson && strategyJson.id && strategyJson.name) {
              ...
              if (!Array.isArray(strategyJson.nodes) || !Array.isArray(strategyJson.edges)) {
                logger.warn(`[StrategyManager] Strategy JSON from ${file} is missing 'nodes' or 'edges' array. Skipping.`);
                return;
              }
              this.strategies.set(strategyJson.id, strategyJson);
              logger.info(`[StrategyManager] Loaded strategy: "${strategyJson.name}" (ID: ${strategyJson.id}) from ${file}`);
            } else {
              logger.warn(`[StrategyManager] Strategy JSON from ${file} is missing 'id' or 'name'. Skipping.`);
            }
          } catch (error) {
            logger.error(`[StrategyManager] Error loading or parsing strategy from ${file}: ${error.message}`, { stack: error.stack });
          }
        }
      });
    } catch (error) {
      logger.error(`[StrategyManager] Error reading strategies directory ${STRATEGIES_DIR_JSON}: ${error.message}`, { stack: error.stack });
    }
    logger.info(`[StrategyManager] Total JSON strategies loaded: ${this.strategies.size}`);
  }
  ...
  getStrategy(id) {
    if (!id) {
      logger.warn('[StrategyManager] Attempted to get strategy with no ID, returning default strategy.');
    ...
    logger.debug(`[StrategyManager] Retrieved strategy: ${strategy.name} (ID: ${id})`);
    return strategy;
  }

  getDefaultStrategy() {
    const defaultStrategyId = config.translationStrategy;
    logger.debug(`[StrategyManager] Default strategy ID from config: ${defaultStrategyId}`);

    let defaultStrategy = this.strategies.get(defaultStrategyId);

    if (!defaultStrategy) {
      logger.error(`[StrategyManager] Default strategy ID "${defaultStrategyId}" from config not found!`);
      if (this.strategies.size > 0) {
        ...
        if (fallbackStrategyEntry) {
            ...
            logger.warn(`[StrategyManager] Using fallback strategy: "${defaultStrategy.name}" (ID: ${defaultStrategy.id})`);
        }
      }
    }

    if (!defaultStrategy) {
      logger.error('[StrategyManager] No strategies loaded or available to use as default. Cannot provide a default strategy.');
      // This is a critical error, as the system cannot function without a strategy.
      throw new MCRError(ErrorCodes.NO_STRATEGY_AVAILABLE, 'No translation strategies available. Ensure at least one JSON strategy is defined and loadable.');
    }

    logger.debug(`[StrategyManager] Retrieved default strategy: "${defaultStrategy.name}" (ID: ${defaultStrategy.id})`);
