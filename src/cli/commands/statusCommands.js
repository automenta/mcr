...
async function getServerStatusCliAsync(options, commandInstance) {
  ...
  if (statusResult.success) {
    handleCliOutput(statusResult.data, programOpts, null, '✅ MCR API Status (Online):\n');
  } else {
    ...
    if (isJsonOutput) {
      ...
      if (statusResult.status === 'offline') {
        logger.info(`🔌 MCR API server not reachable at ${API_BASE_URL}. Status: Offline`);
      } else if (statusResult.status === 'error_response') {
        logger.info(`💥 MCR API server at ${API_BASE_URL} responded with an error. Status: Error`);
        // console.error(`   Details: ${statusResult.message}`); // Message already contains status
        if(statusResult.details) {
            logger.info(`   Server Details: ${typeof statusResult.details === 'string' ? statusResult.details : JSON.stringify(statusResult.details)}`);
        }
      } else { // unknown_error
        logger.info(`❓ Failed to determine MCR API server status at ${API_BASE_URL}: ${statusResult.message}. Status: Unknown`);
      }
    }
  }
