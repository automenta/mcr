...
const { runFamilyOntologyDemoAsync } = require('../../demos/familyOntologyDemo');
...
async function runDemoAction(demoName, options, commandInstance) {
  ...
  if (config.llm && config.llm.provider) {
    ...
    demoLogger.info('LLM Used', `Provider: ${config.llm.provider}, Model: ${config.llm.modelName[config.llm.provider] || 'N/A (Check Config)'}`);
  } else {
    demoLogger.error('LLM configuration not found. Demo might not function correctly.');
  }
  ...
  } else if (demoName.toLowerCase() === 'family' || demoName.toLowerCase() === 'familyontology') {
...
module.exports = (program) => {
  const cmd = program.command('demo')
