...
const { useEffect, useState } = require('react'); // Removed useCallback
...
const App = () => {
  ...
  const [view, setView] = useState('main'); // main, listStrategies, viewStrategyRuns, viewRunDetails, dashboard
    // const [dashboardData, setDashboardData] = useState(null); // Removed unused dashboardData
    ...
    async function fetchStrategies() {
      ...
      try {
        ...
        if (rows && rows.length > 0) {
          setUniqueStrategies(
            rows.map((r) => ({
              label: r.strategy_hash,
              value: r.strategy_hash,
            }))
          );
          setLoadingMessage('');
        } else {
          setLoadingMessage(
            'No strategies found in performance_results database.'
          );
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (selectedStrategyHash && view === 'viewStrategyRuns') {
      async function fetchStrategyRuns() {
        setLoadingMessage(
          `Fetching runs for strategy ${selectedStrategyHash.substring(0, 10)}...`
        );
        ...
        try {
          ...
          if (rows && rows.length > 0) {
            ...
            setLoadingMessage(
              `No runs found for strategy ${selectedStrategyHash.substring(0, 10)}...`
            );
          }
        } catch (err) {
          logger.error(
            `[EvaluatorTUI] Error fetching runs for strategy ${selectedStrategyHash}:`,
            err
          );
  ...
  useInput((input, key) => {
    ...
    if (
      input === 'q' &&
      view !== 'listStrategies' &&
      view !== 'viewStrategyRuns' &&
      view !== 'viewRunDetails'
    ) {
      // Allow 'q' to exit if not in a selection mode
      exit();
    }
  });
  ...
  const handleRunSelect = (item) => {
    // item here is the full run object
    setSelectedRun(item);
  ...
  if (error) {
    return (
      <Box padding={1}>
        <Text color="red">
          Error: {error} (Press Esc to try to go back or Ctrl+C to exit)
        </Text>
      </Box>
    );
  }
  ...
  if (loadingMessage) {
    return (
      <Box padding={1}>
        <Spinner type="dots" /> <Text>{loadingMessage}</Text>
      </Box>
    );
  }
    ...
    <Box flexDirection="column" padding={1} width="100%">
      ...
      {view === 'viewStrategyRuns' && selectedStrategyHash && (
        <Box flexDirection="column">
          ...
          <Text italic>
            (Showing last 20 runs. Select a run to see details.)
          </Text>
          {strategyRuns.length > 0 ? (
            ...
            <SelectInput
              ...
              onSelect={(selectedItem) => {
                const fullRun = strategyRuns.find(
                  (r) => r.id === selectedItem.value
                );
      ...
      {view === 'viewRunDetails' && selectedRun && (
        <Box flexDirection="column" borderStyle="single" padding={1}>
          ...
          <Box marginTop={1}>
            <Text bold>Cost:</Text>
            {selectedRun.cost ? (
              Object.entries(selectedRun.cost).map(([key, value]) => (
                <Text key={key}>
                  {' '}
                  {key}: {String(value)}
                </Text>
          ...
          <Box marginTop={1} flexDirection="column">
            ...
            <Box borderStyle="round" padding={1} width="100%">
              <Text>
                {String(selectedRun.raw_output).substring(0, 1000) +
                  (String(selectedRun.raw_output).length > 1000 ? '...' : '')}
              </Text>
            </Box>
          </Box>
        </Box>
