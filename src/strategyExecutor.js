...
function convertSirToProlog(sirJson, strategyName = 'UnknownStrategy') {
  ...
  const formatTerm = (term) => {
    if (!term || typeof term.predicate !== 'string' || !Array.isArray(term.arguments)) {
      throw new MCRError(ErrorCodes.INVALID_SIR_STRUCTURE, `Invalid term structure in SIR JSON: ${JSON.stringify(term)}`);
    }
    ...
    const formatArgument = (arg) => {
      ...
      if (typeof arg !== 'string') {
        throw new MCRError(ErrorCodes.INVALID_SIR_ARGUMENT, `Argument is not a string, number or array: ${JSON.stringify(arg)}`);
      }
  ...
  const processSingleSirItem = (item) => {
    if (item.statementType === 'fact' && item.fact) {
      ...
      if (item.fact.isNegative) { // Handle isNegative property
        clause = `not(${clause})`;
      }
      clauses.push(`${clause}.`);
      ...
      if (item.rule.body.length === 0) {
        ...
        const bodyStr = item.rule.body
          .map((bodyTerm) => {
            ...
            if (bodyTerm.isNegative) { // Handle isNegative property for body terms
              term = `not(${term})`;
            }
            return term;
          })
      ...
      logger.warn(`[${strategyName}] Invalid SIR structure for item: ${JSON.stringify(item)}`);
  ...
  } else if (sirJson && Object.keys(sirJson).length > 0) { // Check if it's a non-empty object that doesn't match expected structure
    logger.warn(`[${strategyName}] Unexpected SIR JSON format. Expected object with statementType or array of such objects. Received: ${JSON.stringify(sirJson).substring(0, 200)}`);
...
class StrategyExecutor {
  ...
  constructor(strategyJson) {
    if (!strategyJson || !strategyJson.id || !Array.isArray(strategyJson.nodes) || !Array.isArray(strategyJson.edges)) {
      throw new MCRError(ErrorCodes.INVALID_STRATEGY_DEFINITION, `Strategy JSON is invalid or missing required fields (id, nodes, edges). Provided: ${JSON.stringify(strategyJson)}`);
    }
    ...
    logger.info(`[StrategyExecutor] Initialized for strategy: ${this.strategy.id}`);
  }

  _findStartNode() {
    const targetNodeIds = new Set(this.strategy.edges.map(edge => edge.to));
    const startNodes = this.strategy.nodes.filter(node => !targetNodeIds.has(node.id));
    if (startNodes.length === 0) {
      throw new MCRError(ErrorCodes.INVALID_STRATEGY_DEFINITION, `Strategy ${this.strategy.id} has no start node (a node with no incoming edges).`);
    }
    if (startNodes.length > 1) {
      ...
      logger.warn(`[StrategyExecutor] Strategy ${this.strategy.id} has multiple potential start nodes. Using the first one found: ${startNodes[0].id}.`);
    }
    return startNodes[0];
  }
  ...
  async execute(llmProvider, initialContext) {
    logger.info(`[StrategyExecutor] Executing strategy "${this.strategy.id}"...`);
    ...
    while (queue.length > 0) {
      ...
      if (visited.has(currentNodeId)) {
        ...
        logger.debug(`[StrategyExecutor] Skipping already visited node: ${currentNodeId}`);
        continue;
      }

      const node = this.nodeMap.get(currentNodeId);
      if (!node) {
        throw new MCRError(ErrorCodes.INVALID_STRATEGY_DEFINITION, `Node ${currentNodeId} not found in strategy ${this.strategy.id}`);
      }

      logger.debug(`[StrategyExecutor] Processing node: ${node.id} (Type: ${node.type})`);
      ...
      try {
        switch (node.type) {
          case 'LLM_Call':
            if (!node.prompt_template_name) {
              throw new MCRError(ErrorCodes.INVALID_STRATEGY_NODE, `LLM_Call node ${node.id} missing 'prompt_template_name'.`);
            }
            ...
            templateContext.llm_model_id = node.model || initialContext.llm_model_id || 'default_model';
            ...
            logger.debug(`[StrategyExecutor] Node ${node.id}: Preparing LLM call with template ${node.prompt_template_name}. Context keys: ${Object.keys(templateContext).join(', ')}`);

            const userPrompt = fillTemplate(promptTemplate.user, templateContext);
            output = await llmProvider.generate(promptTemplate.system, userPrompt);
            logger.debug(`[StrategyExecutor] Node ${node.id}: LLM call completed. Output length: ${output?.length}`);
            break;

          case 'Parse_JSON':
            if (!node.input_variable) {
              throw new MCRError(ErrorCodes.INVALID_STRATEGY_NODE, `Parse_JSON node ${node.id} missing 'input_variable'.`);
            }
            const jsonString = executionState[node.input_variable];
            if (typeof jsonString !== 'string') {
              throw new MCRError(ErrorCodes.INVALID_NODE_INPUT, `Input for Parse_JSON node ${node.id} (variable '${node.input_variable}') is not a string. Found: ${typeof jsonString}`);
            }
            try {
              ...
              if (!jsonMatch) {
                  ...
                  logger.debug(`[StrategyExecutor] Node ${node.id}: No JSON markdown code block found, attempting to parse entire input as JSON. Input: ${jsonString.substring(0,100)}...`);
                  output = JSON.parse(jsonString);
              } else {
                  const extractedJson = jsonMatch[1] || jsonMatch[2] || jsonMatch[3];
                  output = JSON.parse(extractedJson);
                  logger.debug(`[StrategyExecutor] Node ${node.id}: Extracted and parsed JSON from code block/direct structure.`);
              }

            } catch (e) {
              logger.error(`[StrategyExecutor] Node ${node.id}: Failed to parse JSON string from variable '${node.input_variable}'. Error: ${e.message}. String: ${jsonString.substring(0, 200)}...`);
              throw new MCRError(ErrorCodes.JSON_PARSING_FAILED, `Failed to parse JSON for node ${node.id}: ${e.message}`);
            }
            logger.debug(`[StrategyExecutor] Node ${node.id}: JSON parsing completed.`);
            break;

          case 'SIR_To_Prolog':
            if (!node.input_variable) {
              throw new MCRError(ErrorCodes.INVALID_STRATEGY_NODE, `SIR_To_Prolog node ${node.id} missing 'input_variable'.`);
            }
            const sirJson = executionState[node.input_variable];
            if (typeof sirJson !== 'object' || sirJson === null) {
                throw new MCRError(ErrorCodes.INVALID_NODE_INPUT, `Input for SIR_To_Prolog node ${node.id} (variable '${node.input_variable}') is not an object. Found: ${typeof sirJson}`);
            }
            output = convertSirToProlog(sirJson, this.strategy.name || this.strategy.id); // Pass strategy name for logging
            logger.debug(`[StrategyExecutor] Node ${node.id}: SIR to Prolog conversion completed. Clauses generated: ${output.length}`);
            break;

          case 'Split_String_To_Array':
            if (!node.input_variable) {
              throw new MCRError(ErrorCodes.INVALID_STRATEGY_NODE, `Split_String_To_Array node ${node.id} missing 'input_variable'.`);
            }
            const stringToSplit = executionState[node.input_variable];
            if (typeof stringToSplit !== 'string') {
              throw new MCRError(ErrorCodes.INVALID_NODE_INPUT, `Input for Split_String_To_Array node ${node.id} (variable '${node.input_variable}') is not a string. Found: ${typeof stringToSplit}`);
            }
            const delimiter = node.delimiter || "\n"; // Default to newline
            output = stringToSplit.split(delimiter).map(s => s.trim()).filter(s => s.length > 0);
            logger.debug(`[StrategyExecutor] Node ${node.id}: String splitting completed. Array size: ${output.length}`);
            break;

          case 'Extract_Prolog_Query': // For query strategies that output a query string
            if (!node.input_variable) {
                throw new MCRError(ErrorCodes.INVALID_STRATEGY_NODE, `Extract_Prolog_Query node ${node.id} missing 'input_variable'.`);
            }
            const rawQueryOutput = executionState[node.input_variable];
            if (typeof rawQueryOutput !== 'string') {
                throw new MCRError(ErrorCodes.INVALID_NODE_INPUT, `Input for Extract_Prolog_Query node ${node.id} (variable '${node.input_variable}') is not a string. Found: ${typeof rawQueryOutput}`);
            }
            let cleanedQuery = rawQueryOutput.trim();
            // Remove potential "prolog", "```prolog", "```" wrappers
            cleanedQuery = cleanedQuery.replace(/^(?:prolog\s*)?```(?:prolog)?\s*/i, '').replace(/\s*```$/, '');
            ...
            if (cleanedQuery) {
                ...
                logger.warn(`[StrategyExecutor] Node ${node.id}: LLM generated empty Prolog query after cleaning: "${rawQueryOutput}"`);
            ...
            output = cleanedQuery;
            logger.debug(`[StrategyExecutor] Node ${node.id}: Prolog query extraction completed. Query: "${output}"`);
            break;

          default:
            throw new MCRError(ErrorCodes.UNKNOWN_NODE_TYPE, `Unknown node type "${node.type}" in strategy ${this.strategy.id} for node ${node.id}.`);
        }

        if (node.output_variable) {
          executionState[node.output_variable] = output;
          logger.debug(`[StrategyExecutor] Node ${node.id}: Stored output in variable '${node.output_variable}'.`);
        } else {
          ...
          finalOutput = output;
          logger.debug(`[StrategyExecutor] Node ${node.id}: Output not stored in a variable, considered as potential final output.`);
        }
        ...
        logger.error(`[StrategyExecutor] Error processing node ${node.id} (Type: ${node.type}) in strategy ${this.strategy.id}: ${error.message}`, { stack: error.stack, details: error.details });
    ...
    if (visited.size !== this.strategy.nodes.length) {
      const unvisitedNodes = this.strategy.nodes.filter(n => !visited.has(n.id)).map(n => n.id);
      logger.warn(`[StrategyExecutor] Strategy ${this.strategy.id} execution completed, but not all nodes were visited. Unvisited: ${unvisitedNodes.join(', ')}. This might be normal for conditional graphs.`);
    ...
    let result;
    if (this.strategy.result_variable && executionState.hasOwnProperty(this.strategy.result_variable)) {
        result = executionState[this.strategy.result_variable];
        logger.info(`[StrategyExecutor] Strategy "${this.strategy.id}" execution finished. Returning result from specified 'result_variable': ${this.strategy.result_variable}.`);
    } else if (finalOutput !== undefined) {
        result = finalOutput;
        logger.info(`[StrategyExecutor] Strategy "${this.strategy.id}" execution finished. Returning output of last processed node without an output_variable.`);
    } else {
        logger.warn(`[StrategyExecutor] Strategy "${this.strategy.id}" execution finished, but no clear final output was determined (no 'result_variable' set and last node might have saved its output). Returning entire execution state for debugging.`);
        result = executionState; // Fallback for debugging, not ideal for production
    }

    logger.debug(`[StrategyExecutor] Final execution result for strategy "${this.strategy.id}":`, result);
    return result;
  }
}
