...
class MCREngine {
  constructor() {
    ...
    this.config = {
      ...
      llm: {
        ...
        anthropic: {
          ...
          model: process.env.MCR_LLM_MODEL_ANTHROPIC || 'claude-3-opus-20240229',
        },
        generic_openai: {
    ...
    if (this.embeddingBridge) {
      logger.debug('[MCREngine] EmbeddingBridge instance:', this.embeddingBridge);
    ...
    try {
      ...
      logger.error('[MCREngine] Failed to initialize InputRouter. Routing will be disabled.', error);
  ...
  validateConfig() {
    ...
    if (selectedProvider === 'gemini') {
      ...
      if (!openai.apiKey) {
        throw new Error('Configuration Error: MCR_LLM_PROVIDER is "openai" but OPENAI_API_KEY is not set.');
      }
    } else if (selectedProvider === 'anthropic') {
      if (!anthropic.apiKey) {
        throw new Error('Configuration Error: MCR_LLM_PROVIDER is "anthropic" but ANTHROPIC_API_KEY is not set.');
      }
    } else if (selectedProvider === 'generic_openai') {
      if (!this.config.llm.generic_openai?.model) {
        throw new Error('Configuration Error: MCR_LLM_PROVIDER is "generic_openai" but MCR_LLM_MODEL_GENERIC_OPENAI is not set.');
      }
      if (!this.config.llm.generic_openai?.baseURL) {
        throw new Error('Configuration Error: MCR_LLM_PROVIDER is "generic_openai" but MCR_LLM_GENERIC_OPENAI_BASE_URL is not set.');
      }
    }

    if (this.config.reasoner.type === 'ltn') {
      if (typeof this.config.reasoner.ltnThreshold !== 'number' || this.config.reasoner.ltnThreshold < 0 || this.config.reasoner.ltnThreshold > 1) {
        throw new Error('Configuration Error: REASONER_TYPE is "ltn" but LTN_THRESHOLD is not a float between 0 and 1.');
      }
    }

    const validDebugLevels = ['none', 'basic', 'verbose'];
    if (!validDebugLevels.includes(this.config.debugLevel.toLowerCase())) {
      console.warn(`Warning: Invalid MCR_DEBUG_LEVEL "${this.config.debugLevel}". Allowed values: ${validDebugLevels.join(', ')}. Defaulting to "none".`);
  ...
  async initializeSessionStore() {
    if (this.config.sessionStore.type === 'file') {
      try {
        ...
        logger.info(`[MCREngine] Sessions directory ensured: ${this.sessionsDir}`);
      } catch (error) {
        logger.error(`[MCREngine] Failed to create sessions directory ${this.sessionsDir}:`, error);
        throw error;
      }
    }
  }
  ...
  async _readSessionFile(sessionId) {
    try {
      ...
      logger.error(`[MCREngine] Error reading session file for ${sessionId}:`, error);
      throw error;
    }
  }
  ...
  async _writeSessionFile(sessionId, sessionData) {
    try {
      ...
      logger.error(`[MCREngine] Error writing session file for ${sessionId}:`, error);
      throw error;
    }
  }
  ...
  async createSession(sessionIdInput) {
    ...
    if (this.config.sessionStore.type === 'file') {
      ...
      try {
        ...
        logger.warn(`[MCREngine] Session file ${filePath} already exists for ID: ${sessionId}. Reading existing session.`);
        ...
        if (error.code !== 'ENOENT') {
          logger.error(`[MCREngine] Error checking existence of session file ${filePath}:`, error);
          throw error;
        }
      }
    } else {
      if (this.sessions[sessionId]) {
        logger.warn(`[MCREngine] createSession called with an existing ID: ${sessionId}. Returning existing session.`);
    ...
    if (this.config.sessionStore.type === 'file') {
      ...
      logger.info(`[MCREngine] Session created and file written: ${this._getFilePath(sessionId)}`);
    } else {
  ...
  async translateNLToRulesDirect(naturalLanguageText, strategyIdToUse) {
    ...
    if (!strategyJsonToUse) {
      logger.error(`[MCREngine] No valid strategy found for direct NL to Rules. Base ID: "${effectiveBaseId}".`);
      return {
        success: false,
    ...
    logger.info(`[MCREngine] Enter translateNLToRulesDirect (OpID: ${operationId}). Strategy ID: "${currentStrategyId}". NL Text: "${naturalLanguageText}"`);

    try {
      logger.info(`[MCREngine] Using strategy "${strategyJsonToUse.name}" (ID: ${currentStrategyId}) for direct NL to Rules. OpID: ${operationId}`);
      ...
      const prologRules = await new StrategyExecutor(strategyJsonToUse).execute(this, this, initialContext);

      if (!Array.isArray(prologRules) || !prologRules.every(r => typeof r === 'string')) {
        logger.error(`[MCREngine] Strategy "${currentStrategyId}" execution for direct translation did not return an array of strings. OpID: ${operationId}. Output: ${JSON.stringify(prologRules)}`);
      ...
      logger.debug(`[MCREngine] Strategy "${currentStrategyId}" execution returned (OpID: ${operationId}):`, { prologRules });

      if (!prologRules || prologRules.length === 0) {
        logger.warn(`[MCREngine] Strategy "${currentStrategyId}" extracted no rules from text (OpID: ${operationId}): "${naturalLanguageText}"`);
        return {
          success: false,
      ...
      logger.info(`[MCREngine] Successfully translated NL to Rules (Direct). OpID: ${operationId}. Rules count: ${prologRules.length}. Strategy ID: ${currentStrategyId}`);
      return { success: true, rules: prologRules, strategyId: currentStrategyId };
    } catch (error) {
      logger.error(`[MCREngine] Error translating NL to Rules (Direct) using strategy "${currentStrategyId}" (OpID: ${operationId}): ${error.message}`, { stack: error.stack, details: error.details, errorCode: error.code });
      return {
        success: false,
  ...
  async translateRulesToNLDirect(prologRules, style = 'conversational') {
    ...
    logger.info(`[MCREngine] Enter translateRulesToNLDirect (OpID: ${operationId}). Style: ${style}. Rules length: ${prologRules?.length}`);
    logger.debug(`[MCREngine] Rules for direct translation to NL (OpID: ${operationId}):\n${prologRules}`);

    if (!prologRules || typeof prologRules !== 'string' || prologRules.trim() === '') {
      logger.warn(`[MCREngine] translateRulesToNLDirect called with empty or invalid prologRules. OpID: ${operationId}`);
      return {
        success: false,
    ...
    try {
      ...
      logger.info(`[MCREngine] Generating NL explanation from rules using LLM. OpID: ${operationId}`);
      logger.debug(`[MCREngine] Context for RULES_TO_NL_DIRECT prompt (OpID: ${operationId}):`, promptContext);
      const rulesToNLPromptUser = fillTemplate(directRulesToNlPrompt.user, promptContext);

      const llmExplanationResult = await this.callLLM(directRulesToNlPrompt.system, rulesToNLPromptUser);
      ...
      logger.debug(`[MCREngine] Prolog rules translated to NL (Direct) (OpID: ${operationId}):\n${nlExplanationText}`);

      if (nlExplanationText === null || (typeof nlExplanationText === 'string' && nlExplanationText.trim() === '')) {
        logger.warn(`[MCREngine] Empty explanation generated for rules to NL (Direct). OpID: ${operationId}`);
        return {
          success: false,
      ...
      logger.info(`[MCREngine] Successfully translated Rules to NL (Direct). OpID: ${operationId}. Explanation length: ${nlExplanationText.length}.`);
      return { success: true, explanation: nlExplanationText };
    } catch (error) {
      logger.error(`[MCREngine] Error translating Rules to NL (Direct) (OpID: ${operationId}): ${error.message}`, { error: error.stack });
      return {
        success: false,
  ...
  async explainQuery(sessionId, naturalLanguageQuestion) {
    ...
    logger.info(`[MCREngine] Enter explainQuery for session ${sessionId} (OpID: ${operationId}). Strategy: "${activeStrategyJson.name}" (ID: ${currentStrategyId}). NL Question: "${naturalLanguageQuestion}"`);
    ...
    if (!explainPrologQueryPrompt) {
      logger.error('[MCREngine] EXPLAIN_PROLOG_QUERY prompt template not found.');
      return {
        success: false,
    ...
    try {
      ...
      try {
        ...
        logger.warn(`[MCREngine] Error fetching global ontologies for NL_TO_QUERY context in explain (OpID: ${operationId}): ${ontError.message}`);
      ...
      logger.info(`[MCREngine] Executing strategy "${activeStrategyJson.name}" (ID: ${currentStrategyId}) for query translation in explain. OpID: ${operationId}.`);

      const prologQuery = await new StrategyExecutor(activeStrategyJson).execute(this, this, initialStrategyContext);

      if (typeof prologQuery !== 'string' || !prologQuery.endsWith('.')) {
        logger.error(`[MCREngine] Strategy "${currentStrategyId}" execution for explain query did not return a valid Prolog query string. OpID: ${operationId}. Output: ${prologQuery}`);
      ...
      logger.info(`[MCREngine] Strategy "${currentStrategyId}" translated NL to Prolog query for explanation (OpID: ${operationId}): ${prologQuery}`);
      ...
      try {
        ...
        logger.warn(`[MCREngine] Error fetching global ontologies for EXPLAIN_PROLOG_QUERY prompt context (OpID: ${operationId}): ${ontErrorForExplain.message}`);
      ...
      const llmExplanationResult = await this.callLLM(explainPrologQueryPrompt.system, fillTemplate(explainPrologQueryPrompt.user, explainPromptContext));
      ...
      logger.error(`[MCREngine] Error explaining query for session ${sessionId} (OpID: ${operationId}, Strategy ID: ${currentStrategyId}): ${error.message}`, { stack: error.stack, details: error.details, errorCode: error.code });
  ...
  async getPrompts() {
    ...
    try {
      logger.debug(`[MCREngine] Successfully retrieved prompts. OpID: ${operationId}. Prompt count: ${Object.keys(prompts).length}`);
      return { success: true, prompts: prompts };
    } catch (error) {
      logger.error(`[MCREngine] Error retrieving prompts (OpID: ${operationId}): ${error.message}`, { error: error.stack });
      return {
        success: false,
  ...
  async debugFormatPrompt(templateName, inputVariables) {
    ...
    logger.info(`[MCREngine] Enter debugFormatPrompt (OpID: ${operationId}). Template: ${templateName}`, { inputVariables });

    if (!templateName || typeof templateName !== 'string') {
      logger.warn(`[MCREngine] Invalid template name for debugFormatPrompt. OpID: ${operationId}`, { templateName });
      return {
        success: false,
    ...
    if (!inputVariables || typeof inputVariables !== 'object') {
      logger.warn(`[MCREngine] Invalid input variables for debugFormatPrompt (OpID: ${operationId}). Received: ${typeof inputVariables}`, { inputVariables });
      return {
        success: false,
    ...
    if (!template) {
      logger.warn(`[MCREngine] Prompt template "${templateName}" not found for debugFormatPrompt. OpID: ${operationId}`);
      return {
        success: false,
    ...
    if (!template.user) {
      logger.warn(`[MCREngine] Prompt template "${templateName}" has no 'user' field for debugFormatPrompt. OpID: ${operationId}`);
      return {
        success: false,
    ...
    try {
      logger.debug(`[MCREngine] Attempting to fill template "${templateName}" with variables. OpID: ${operationId}`);
      const formattedUserPrompt = fillTemplate(template.user, inputVariables);
      logger.info(`[MCREngine] Successfully formatted prompt "${templateName}". OpID: ${operationId}`);
      return {
        success: true,
        templateName,
      ...
      logger.error(`[MCREngine] Error formatting prompt ${templateName} (OpID: ${operationId}): ${error.message}`, { error: error.stack });
      return {
        success: false,
  ...
  async llmPassthrough(naturalLanguageText) {
    ...
    logger.info(`[MCREngine] Enter llmPassthrough (OpID: ${operationId}). NL Text: "${naturalLanguageText}"`);
    ...
    try {
      ...
      const llmPassthroughPromptUser = fillTemplate(llmPassthroughPrompt.user, promptContext);

      const llmResult = await this.callLLM(llmPassthroughPrompt.system, llmPassthroughPromptUser);
      ...
      if (responseText === null || (typeof responseText === 'string' && responseText.trim() === '')) {
        logger.warn(`[MCREngine] Empty response generated for llmPassthrough. OpID: ${operationId}`);
        return {
          success: false,
      ...
      logger.info(`[MCREngine] Successfully generated response for llmPassthrough. OpID: ${operationId}. Response length: ${responseText.length}.`);
      return { success: true, response: responseText };
    } catch (error) {
      logger.error(`[MCREngine] Error in llmPassthrough (OpID: ${operationId}): ${error.message}`, { error: error.stack });
      return {
        success: false,
  ...
  async setSessionKnowledgeBase(sessionId, kbContent) {
    ...
    logger.info(`[MCREngine] Enter setSessionKnowledgeBase for session ${sessionId}. OpID: ${operationId}. KB length: ${kbContent.length}`);

    const sessionExists = await this.getSession(sessionId);
    if (!sessionExists) {
      logger.warn(`[MCREngine] Session ${sessionId} not found for setKnowledgeBase. OpID: ${operationId}`);
      return {
        success: false,
        message: 'Session not found.',
        error: ErrorCodes.SESSION_NOT_FOUND,
    ...
    try {
      if (typeof kbContent !== 'string') {
        logger.error(`[MCREngine] Invalid kbContent type for setKnowledgeBase. OpID: ${operationId}. Type: ${typeof kbContent}`);
        return {
          success: false,
      ...
      if (success) {
        ...
        logger.info(`[MCREngine] Knowledge base for session ${sessionId} successfully replaced. OpID: ${operationId}`);
        return {
          success: true,
        ...
        logger.error(`[MCREngine] Failed to set knowledge base in session store for session ${sessionId}. OpID: ${operationId}`);
        return {
          success: false,
      ...
      logger.error(`[MCREngine] Error setting knowledge base for session ${sessionId} (OpID: ${operationId}): ${error.message}`, { stack: error.stack, details: error.details, errorCode: error.code });
      return {
        success: false,
  ...
  async assertRawPrologToSession(sessionId, rules, validate = true) {
    ...
    logger.info(`[MCREngine] Enter assertRawPrologToSession for session ${sessionId}. OpID: ${operationId}. Rules count/length: ${Array.isArray(rules) ? rules.length : rules.length}`);

    const sessionExists = await this.getSession(sessionId);
    if (!sessionExists) {
      logger.warn(`[MCREngine] Session ${sessionId} not found for raw Prolog assertion. OpID: ${operationId}`);
      return {
        success: false,
        message: 'Session not found.',
        error: ErrorCodes.SESSION_NOT_FOUND,
    ...
    if (factsToAssert.length === 0) {
      logger.warn(`[MCREngine] No valid Prolog facts/rules provided to assert. OpID: ${operationId}`);
      return {
        success: false,
    ...
    if (validate) {
      for (const factString of factsToAssert) {
        ...
        if (!validationResult.isValid) {
          ...
          logger.error(`[MCREngine] Validation failed for provided Prolog. OpID: ${operationId}. Details: ${validationErrorMsg}`);
          return {
            success: false,
      ...
      logger.info(`[MCREngine] All ${factsToAssert.length} provided Prolog snippets validated successfully. OpID: ${operationId}`);
    }
    ...
    try {
      ...
      if (addSuccess) {
        const fullKnowledgeBase = await this.getKnowledgeBase(sessionId);
        logger.info(`[MCREngine] Raw Prolog facts/rules successfully added to session ${sessionId}. OpID: ${operationId}. Count: ${factsToAssert.length}`);
        return {
          success: true,
        ...
        logger.error(`[MCREngine] Failed to add raw Prolog to session ${sessionId} after validation/processing. OpID: ${operationId}`);
        return {
          success: false,
      ...
      logger.error(`[MCREngine] Error asserting raw Prolog to session ${sessionId} (OpID: ${operationId}): ${error.message}`, { stack: error.stack, details: error.details, errorCode: error.code });
      return {
        success: false,
  ...
  async addFacts(sessionId, newFacts) {
    ...
    if (!sessionData) {
        logger.warn(`[MCREngine] Cannot add facts: Session not found: ${sessionId}`);
        return false;
    }
    if (!Array.isArray(newFacts) || !newFacts.every(f => typeof f === 'string')) {
        logger.warn(`[MCREngine] Cannot add facts: newFacts must be an array of strings. Session: ${sessionId}`);
        return false;
    }

    const validatedFacts = newFacts.map(f => String(f).trim()).filter(f => f.length > 0 && f.endsWith('.'));
    ...
    logger.info(`[MCREngine] ${validatedFacts.length} facts added to session: ${sessionId}. Total facts: ${sessionData.facts.length}. Lexicon size: ${sessionData.lexicon.size}`);
    return true;
  }
    ...
    facts.forEach(fact => {
        ...
        if (structuredTermMatch) {
            ...
            if (simpleAtomMatch) {
                ...
                logger.debug(`[MCREngine] Could not parse predicate/arity from term: ${termToParse} in session ${sessionData.id}`);
            }
        }
    });
  }
  ...
  async getKnowledgeBase(sessionId) {
    ...
    if (!sessionData) {
        logger.warn(`[MCREngine] Cannot get knowledge base: Session not found: ${sessionId}`);
        return null;
    }
  ...
  async deleteSession(sessionId) {
    if (this.config.sessionStore.type === 'file') {
        ...
        try {
            ...
            if (error.code === 'ENOENT') {
                logger.warn(`[MCREngine] Cannot delete session: File not found: ${filePath}`);
                return false;
            }
            logger.error(`[MCREngine] Error deleting session file ${filePath}:`, error);
            throw error;
        }
    } else {
        if (!this.sessions[sessionId]) {
            logger.warn(`[MCREngine] Cannot delete session: Session not found: ${sessionId}`);
            return false;
        }
  ...
  async getLexiconSummary(sessionId) {
    ...
    if (!sessionData) {
        logger.warn(`[MCREngine] Cannot get lexicon summary: Session not found: ${sessionId}`);
        return null;
    }
  ...
  async listSessions() {
    if (this.config.sessionStore.type === 'file') {
        try {
            ...
            const sessionsData = await Promise.all(
                sessionFiles.map(async file => {
                    ...
                    try {
                        ...
                        logger.warn(`[MCREngine] Could not read or parse session file ${file}: ${error.message}`);
                        return null;
                    }
                    return null;
                })
            ...
            logger.error(`[MCREngine] Error listing sessions from directory ${this.sessionsDir}:`, error);
            return [];
        }
    } else {
    ...
    if (!this.llmProvider) {
      ...
      switch (providerName) {
        ...
        case 'gemini':
          ...
          break;
        default:
          throw new Error(`Configuration Error: Unsupported LLM provider configured: "${providerName}". Supported providers are "ollama", "gemini".`);
      }
      logger.info(`[MCREngine] LLM Service initialized with provider: ${this.llmProvider.name}.`);
    }
  ...
  async callLLM(systemPrompt, userPrompt, options = {}, input = {}) {
    ...
    try {
      ...
      if (typeof result === 'string') {
        logger.warn(`[MCREngine] Provider ${provider.name} returned a string instead of {text, costData} object. Assuming no cost data.`);
        return { text: result, costData: null };
      }
      return result;
    } catch (error) {
      logger.error(`[MCREngine] Error during LLM generation with ${provider.name}: ${error.message}`, {
  ...
  getReasonerProvider() {
    if (!this.reasonerProvider) {
      ...
      logger.info(`[MCREngine] Reasoner Service initialized with provider: ${this.reasonerProvider.name}`);
    }
  ...
  async queryProlog(knowledgeBase, query, options = {}) {
    ...
    try {
      logger.debug(`[MCREngine] queryProlog called with provider ${provider.name}`, { knowledgeBaseLen: knowledgeBase.length, query, options });
      return await provider.executeQuery(knowledgeBase, query, options);
    } catch (error) {
      logger.error(`[MCREngine] Error during reasoner execution with ${provider.name}: ${error.message}`, {
        provider: provider.name,
        query,
        error,
      });
  ...
  async guidedDeduce(query, llmService, embeddingBridge, session) {
    ...
    for (const hypothesis of hypotheses) {
      ...
      if (result.results.length > 0) {
        const probability = embeddingBridge && result.results[0].embedding ? await embeddingBridge.similarity(await embeddingBridge.encode(query), result.results[0].embedding) : 0.9;
  ...
  async validateKnowledgeBase(knowledgeBase) {
    ...
    if (!provider || typeof provider.validate !== 'function') {
      logger.error('[MCREngine] Reasoner provider is not correctly configured or does not support validate.');
      throw new Error('Reasoner provider misconfiguration for validate.');
    }
    try {
      logger.debug(`[MCREngine] validateKnowledgeBase called with provider ${provider.name}`);
      return await provider.validate(knowledgeBase);
    } catch (error) {
      logger.error(`[MCREngine] Error during reasoner validation with ${provider.name}: ${error.message}`, { provider: provider.name, error });
      throw error;
    }
  }
  ...
  async getOperationalStrategyJson(operationType, naturalLanguageText, multi = true) {
    ...
    const llmModelId = this.config.llm[this.config.llm.provider]?.model || 'default';
    ...
    if (this.inputRouterInstance && naturalLanguageText) {
      try {
        ...
        logger.error(`[MCREngine] InputRouter failed: ${routerError.message}. Falling back.`);
      }
    }
    ...
    if (!strategyJson) {
      ...
      if (strategyJson) {
        logger.info(`[MCREngine] Using configured operational strategy: "${strategyJson.id}"`);
      } else {
        logger.warn(`[MCREngine] Operational strategy "${operationalStrategyId}" not found. Trying base strategy "${this.baseStrategyId}".`);
        strategyJson = strategyManager.getStrategy(this.baseStrategyId) || strategyManager.getDefaultStrategy();
        if (strategyJson) {
          logger.info(`[MCREngine] Using fallback strategy: "${strategyJson.id}"`);
        } else {
          logger.error(`[MCREngine] Failed to initialize with a default assertion strategy. Base ID: "${this.baseStrategyId}".`);
        }
      }
    }
    return strategyJson;
  ...
  async logInitialStrategy() {
    try {
      const initialDisplayStrategy = await this.getOperationalStrategyJson('Assert', 'System startup initial strategy check.');
      logger.info(`[MCREngine] Initialized with base translation strategy ID: "${this.baseStrategyId}". Effective assertion strategy: "${initialDisplayStrategy.name}" (ID: ${initialDisplayStrategy.id})`);
    } catch (e) {
      logger.error(`[MCREngine] Failed to initialize with a default assertion strategy. Base ID: "${this.baseStrategyId}". Error: ${e.message}`);
    }
  }

  async setTranslationStrategy(strategyId) {
    logger.debug(`[MCREngine] Attempting to set base translation strategy ID to: ${strategyId}`);
    ...
    if (assertStrategyExists || queryStrategyExists || baseStrategyItselfExists) {
      ...
      try {
        const currentAssertStrategy = await this.getOperationalStrategyJson('Assert', 'Strategy set check.');
        logger.info(`[MCREngine] Base translation strategy ID changed from "${oldBaseStrategyId}" to "${this.baseStrategyId}". Effective assertion strategy: "${currentAssertStrategy.name}" (ID: ${currentAssertStrategy.id})`);
      } catch (e) {
        logger.warn(`[MCREngine] Base translation strategy ID changed from "${oldBaseStrategyId}" to "${this.baseStrategyId}", but failed to determine effective assertion strategy for logging: ${e.message}`);
      }
      return true;
    }

    logger.warn(`[MCREngine] Attempted to set unknown or invalid base strategy ID: ${strategyId}. Neither "${assertVariantId}", "${queryVariantId}" nor the base ID "${strategyId}" itself were found. Available strategies: ${JSON.stringify(strategyManager.getAvailableStrategies())}`);
    return false;
  }
  ...
  async _refineLoop(operation, initialInput, context, maxIter = 3) {
    ...
    logger.info(`[MCREngine] Starting refinement loop for ${operation.name}. Max iterations: ${maxIter}`);
    ...
    while (iteration <= maxIter) {
      ...
      const validation = await this.validateKnowledgeBase(Array.isArray(currentResult) ? currentResult.join('\n') : currentResult);

      if (validation.isValid) {
        logger.info(`[RefineLoop] Validation successful on iteration ${iteration}. Loop terminated.`);
        return {
      ...
      logger.warn(`[RefineLoop] Validation failed on iteration ${iteration}: ${validation.error}`);
      issues.push({ iteration, error: validation.error });

      if (iteration < maxIter) {
        const similarContext = {};
        if (embeddingBridge && session.embeddings.size > 0) {
          const inputEmbedding = await embeddingBridge.encode(Array.isArray(currentResult) ? currentResult.join(' ') : currentResult);
          let bestMatch = null;
          let maxSim = -1;
          for (const [text, embedding] of session.embeddings.entries()) {
            const sim = await embeddingBridge.similarity(inputEmbedding, embedding);
            if (sim > maxSim) {
              maxSim = sim;
              bestMatch = text;
            }
        ...
        const llmRefinement = await this.callLLM(refinePrompt.system, fillTemplate(refinePrompt.user, refinePromptContext));

        if (llmRefinement.text) {
          logger.info(`[RefineLoop] LLM provided a refinement.`);
          currentResult = llmRefinement.text;
        } else {
          logger.warn(`[RefineLoop] LLM did not provide a refinement. Breaking loop.`);
          break;
        }
      }
    ...
    logger.warn(`[RefineLoop] Loop finished after ${maxIter} iterations without converging.`);
    return {
      result: currentResult,
      iterations: maxIter,
      converged: false,
  ...
  async assertNLToSession(sessionId, naturalLanguageText, options = {}) {
    ...
    if (!activeStrategyJson) {
      throw new MCRError(ErrorCodes.STRATEGY_NOT_FOUND, `Active strategy not found for session ${sessionId}. Please ensure a valid strategy is selected.`);
    }
    const currentStrategyId = activeStrategyJson.id;
    logger.info(`[MCREngine] Enter assertNLToSession for session ${sessionId} using strategy "${activeStrategyJson.name}" (ID: ${currentStrategyId}). NL Text: "${naturalLanguageText}"`);
    const operationId = `assert-${Date.now()}`;

    const session = await this.getSession(sessionId);
    if (!session) {
      logger.warn(`[MCREngine] Session ${sessionId} not found for assertion. OpID: ${operationId}`);
      return {
        success: false,
        message: 'Session not found.',
        error: ErrorCodes.SESSION_NOT_FOUND,
    ...
    try {
      ...
      if (addSuccess) {
        ...
        throw new MCRError(ErrorCodes.SESSION_ADD_FACTS_FAILED, 'Failed to add facts to session store after validation.');
      }
    } catch (error) {
      logger.error(`[MCREngine] Error asserting NL to session ${sessionId}: ${error.message}`, { stack: error.stack });
      return {
        success: false,
  ...
  async querySessionWithNL(sessionId, naturalLanguageQuestion, queryOptions = {}) {
    const { dynamicOntology, style = 'conversational', trace = false, useLoops = true } = queryOptions;
    ...
    logger.info(`[MCREngine] Enter querySessionWithNL for session ${sessionId} using strategy "${activeStrategyJson.name}" (ID: ${currentStrategyId}). NL Question: "${naturalLanguageQuestion}"`, { queryOptions });
    ...
    try {
      ...
      if (useLoops) {
        const loopResult = await this._refineLoop(nlToQueryOperation, naturalLanguageQuestion, { session, embeddingBridge: this.embeddingBridge }, 2);
      ...
      if (typeof prologQuery !== 'string' || !prologQuery.endsWith('.')) {
        throw new MCRError(ErrorCodes.STRATEGY_INVALID_OUTPUT, 'Strategy for query generation did not return a valid Prolog query string.');
      }
      debugInfo.prologQuery = prologQuery;
      ...
      const reasonerResult = await this.guidedDeduce(prologQuery, this, this.embeddingBridge, session);
      ...
      const logicToNlOperation = async symbolicResultStr => {
        ...
        const llmResult = await this.callLLM(prompts.LOGIC_TO_NL_ANSWER.system, fillTemplate(prompts.LOGIC_TO_NL_ANSWER.user, promptContext));
      ...
      if (useLoops) {
        naturalLanguageAnswerText = await logicToNlOperation(JSON.stringify(prologResults));
      } else {
        naturalLanguageAnswerText = await logicToNlOperation(JSON.stringify(prologResults));
      }

      if (!naturalLanguageAnswerText) {
        throw new MCRError(ErrorCodes.LLM_EMPTY_RESPONSE, 'Failed to generate a natural language answer.');
      }
      ...
      if (trace && proofTrace) {
        ...
        if (tracePrompt) {
          const llmTraceResult = await this.callLLM(tracePrompt.system, fillTemplate(tracePrompt.user, { trace: JSON.stringify(proofTrace, null, 2) }));
      ...
      logger.error(`[MCREngine] Error querying session ${sessionId}: ${error.message}`, { stack: error.stack });
      debugInfo.error = error.message;
      return {
        success: false,
