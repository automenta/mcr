...
const { readFileContentSafe, delay, parseTuiCommandArgs } = require('./cli/utils');
...
async function runDemo(demoName, options, command) {
  ...
  try {
    if (demoName === 'simpleQA' || demoName === 'simpleqa') {
      ...
      demoContext.logMessage('error', `Unknown demo: ${demoName}. Available: simpleQA, family`);
...
program
  ...
  .action(runDemo); // runDemo is the action handler for 'run'
...
if (require.main === module) {
  ...
  standaloneProgram
    .command('run <demoName>')
    .description('Run a specific demo. Available: simpleQA, family')
    .action(async (demoName, options, command) => { // Correct signature
      ...
      await runDemo(demoName, options, command);
    });

  standaloneProgram.parse(process.argv);
} else {
