...
async function generate(systemPrompt, userPrompt, options = {}) {
  ...
  // let fullPromptContent = systemPrompt // UNUSED
  //   ? `${systemPrompt}\n\n${userPrompt}`
  //   : userPrompt;
  ...
  if (options.jsonMode) {
    ...
    // Note: If jsonMode needs to modify the actual prompt sent, it should modify 'userPrompt' or 'systemPrompt'
    // or be handled by constructing 'messages' array appropriately.
    // For now, commenting out as fullPromptContent is unused.
    // A more direct way for JSON mode is to ensure the system/user prompt itself asks for JSON.
  }
