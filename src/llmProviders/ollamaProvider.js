 ...
 */
async function checkOllamaConnection(ollamaBaseUrl, modelName) {
  try {
    // First, check basic connectivity, e.g., by hitting the version endpoint
    await axios.get(`${ollamaBaseUrl}/api/version`, { timeout: 2000 });
    logger.debug(`Successfully connected to Ollama server at ${ollamaBaseUrl}.`);
    ...
    try {
      await axios.post(`${ollamaBaseUrl}/api/show`, { name: modelName }, { timeout: 3000 });
      logger.info(`Ollama model '${modelName}' is available on server ${ollamaBaseUrl}.`);
    } catch (modelError) {
...
const OllamaProvider = {
  name: 'ollama',
  initialize: async (llmConfig) => { // Made initialize async
    ...
    await checkOllamaConnection(ollamaBaseUrl, model.ollama);
    ...
    try {
      ...
      return client;
    } catch (error) { // This catch might be for unexpected constructor errors
      logger.error(
