...
const { prompts, fillTemplate, getPromptTemplateByName, addOrUpdatePromptTemplate } = require('../prompts'); // Need to manage prompts
...
class StrategyEvolver {
    ...
    selectNodeForMutation(strategyJson) {
        if (!strategyJson || !Array.isArray(strategyJson.nodes)) {
            logger.warn('[StrategyEvolver] Invalid strategy JSON or no nodes array found.');
            return null;
        }
        // For now, select the first LLM_Call node
        const llmNode = strategyJson.nodes.find(node => node.type === 'LLM_Call' && node.prompt_template_name);
        if (!llmNode) {
            logger.warn('[StrategyEvolver] No LLM_Call node with a prompt_template_name found in the strategy.');
            return null;
        }
        logger.info(`[StrategyEvolver] Selected node for mutation: ${llmNode.id} (template: ${llmNode.prompt_template_name})`);
        return llmNode;
    }
    ...
    async getFailingExamples(strategyHash, limit = 3) {
        logger.info(`[StrategyEvolver] Fetching failing examples for strategy hash: ${strategyHash}`);
        try {
            ...
            const rows = await new Promise((resolve, reject) => {
                ...
                db.all(query, [strategyHash, limit], (err, rows) => {
                    if (err) {
                        logger.error(`[StrategyEvolver] Error querying failing examples: ${err.message}`);
                        return reject(err);
                    }
                    resolve(rows);
                });
            ...
            if (!rows || rows.length === 0) {
                logger.info(`[StrategyEvolver] No failing examples found for strategy hash ${strategyHash} based on current criteria.`);
                return [];
            }
            ...
            const enrichedExamples = rows.map(row => {
                const evalCase = evalCaseMap.get(row.example_id);
                if (evalCase) {
                    return {
                        exampleId: row.example_id,
                        naturalLanguageInput: evalCase.naturalLanguageInput,
                        expectedOutput: evalCase.expectedProlog || evalCase.expectedAnswer, // Depending on type
            ...
            logger.info(`[StrategyEvolver] Found ${enrichedExamples.length} enriched failing examples.`);
            return enrichedExamples;

        } catch (error) {
            logger.error(`[StrategyEvolver] Error getting failing examples: ${error.message}`, { stack: error.stack });
            return [];
        }
    }
     ...
     */
    async critiqueAndRewritePrompt(originalPromptContent, failingExamples, promptGoal = "Translate natural language to a specific structured format.") {
        if (failingExamples.length === 0) {
            logger.info("[StrategyEvolver] No failing examples provided for critique. Cannot rewrite prompt.");
            // Optionally, could try a generic rewrite without examples, e.g., "Make this prompt more robust."
            return null;
        }

        const examplesString = failingExamples.map((ex, idx) => {
...
---`;
        ...
        if (!critiquePromptTemplate) {
            logger.error("[StrategyEvolver] CRITIQUE_AND_REWRITE_PROMPT template not found in prompts.js. Cannot proceed.");
            ...
            const fallbackSystem = "You are an expert prompt engineer. Your task is to critique and rewrite a given prompt to improve its performance based on examples where it failed.";
...
`;
            ...
            const rewrittenPrompt = await llmService.generate(fallbackSystem, fallbackUser);
        ...
        const systemPrompt = critiquePromptTemplate.system || "You are an expert prompt engineer. Rewrite the given prompt to address the failures shown in the examples.";

        try {
            logger.info("[StrategyEvolver] Sending prompt for critique and rewrite to LLM...");
            ...
            if (llmResponse) {
                ...
                logger.warn("[StrategyEvolver] LLM returned empty response for prompt rewrite.");
                return null;
            }
        } catch (error) {
            logger.error(`[StrategyEvolver] LLM call for prompt rewrite failed: ${error.message}`, { stack: error.stack });
            return null;
        }
    }
     ...
     */
    createEvolvedStrategyJson(originalStrategyJson, targetNodeId, newPromptContent) {
        ...
        newStrategy.source_strategy_hash = crypto.createHash('sha256').update(JSON.stringify(originalStrategyJson)).digest('hex'); // Track lineage

        const nodeToUpdate = newStrategy.nodes.find(node => node.id === targetNodeId);
        if (!nodeToUpdate) {
            logger.error(`[StrategyEvolver] Node ${targetNodeId} not found in new strategy copy. This should not happen.`);
            return null;
        }
        ...
        logger.info(`[StrategyEvolver] Updated node ${targetNodeId} to use new dynamic prompt template: ${newPromptTemplateName}`);
    ...
    async evolveIterativeCritique(strategyJson) {
        logger.info(`[StrategyEvolver] Starting Iterative Critique for strategy: ${strategyJson.id}`);

        const originalStrategyHash = crypto.createHash('sha256').update(JSON.stringify(strategyJson)).digest('hex');

        const nodeToMutate = this.selectNodeForMutation(strategyJson);
        if (!nodeToMutate) {
            logger.warn('[StrategyEvolver] Could not select a node for mutation. Evolution aborted.');
            return null;
        }

        const originalPromptTemplateName = nodeToMutate.prompt_template_name;
        const originalPromptObject = getPromptTemplateByName(originalPromptTemplateName);
        if (!originalPromptObject || !originalPromptObject.user) { // Check for user part, assuming system might be optional or global
            logger.error(`[StrategyEvolver] Could not retrieve original prompt content for template: ${originalPromptTemplateName}`);
            return null;
        }
        ...
        const promptGoal = originalPromptObject.description || `Goal for prompt template ${originalPromptTemplateName}`;


        const failingExamples = await this.getFailingExamples(originalStrategyHash);
        if (failingExamples.length === 0) {
            logger.info('[StrategyEvolver] No failing examples found for this strategy. Iterative critique cannot proceed effectively without them.');
            // Could add a fallback mutation here, e.g., a generic prompt improvement without examples.
            return null;
        }

        const rewrittenPromptContent = await this.critiqueAndRewritePrompt(originalPromptContent, failingExamples, promptGoal);
        if (!rewrittenPromptContent) {
            logger.warn('[StrategyEvolver] Prompt rewriting failed or returned no content.');
            return null;
        }

        const newStrategyJson = this.createEvolvedStrategyJson(strategyJson, nodeToMutate.id, rewrittenPromptContent);
        if (newStrategyJson) {
            logger.info(`[StrategyEvolver] Successfully created new evolved strategy: ${newStrategyJson.id}`);
            return newStrategyJson;
        } else {
            logger.error('[StrategyEvolver] Failed to create new strategy JSON from rewritten prompt.');
            return null;
        }
    }
