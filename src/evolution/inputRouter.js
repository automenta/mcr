...
class InputRouter {
  ...
  classifyInput(naturalLanguageText) {
    ...
    if (/\?$/.test(nlLower)) { // Ends with ?
      logger.debug(`[InputRouter] Classified input as '${DEFAULT_QUERY_CLASS}': "${naturalLanguageText.substring(0, 50)}..."`);
      return DEFAULT_QUERY_CLASS;
    }

    // Check for specific question-starting keywords.
    // Refined to kw + ' ' to avoid matching parts of words (e.g. "is" in "this").
    // Also check for keyword alone (e.g. "is" or "what?")
    const questionKeywords = [
      'who', 'what', 'where', 'when', 'why', 'how',
      'is', 'are', 'does', 'do', 'can', 'could', 'would', 'should'
    ];

    if (questionKeywords.some(kw => nlLower.startsWith(kw + ' ') || nlLower === kw)) {
      logger.debug(`[InputRouter] Classified input as '${DEFAULT_QUERY_CLASS}': "${naturalLanguageText.substring(0, 50)}..."`);
      return DEFAULT_QUERY_CLASS;
    }

    logger.debug(`[InputRouter] Classified input as '${DEFAULT_ASSERT_CLASS}': "${naturalLanguageText.substring(0, 50)}..."`);
  ...
  async getBestStrategy(inputClass, llmModelId) {
    ...
    try {
      ...
      const targetInputType = inputClass === DEFAULT_ASSERT_CLASS ? 'assert' : 'query';
      ...
      const relevantResults = await this.db.queryPerformanceResults(query, [llmModelId, targetInputType]);

      if (!relevantResults || relevantResults.length === 0) {
        logger.info(`[InputRouter] No performance results found for llmModelId "${llmModelId}" (or generic) and input_type "${targetInputType}".`);
        return null;
      }
      ...
      for (const row of relevantResults) {
        try {
          ...
          if (metrics.exactMatchAnswer === 1 || metrics.exactMatchAnswer === true) successScore +=1; // Relevant for queries
          ...
          const latencyScore = row.latency_ms > 0 ? 1000 / (row.latency_ms + 1) : 1; // Normalize, higher is better
          ...
          if (!strategyScores.has(row.strategy_hash)) {
            strategyScores.set(row.strategy_hash, { totalScore: 0, count: 0, totalLatency: 0, totalCostTokens: 0, successCount: 0 });
          }
          ...
          logger.warn(`[InputRouter] Failed to parse metrics/cost for a row or calculate score: ${e.message}`, { strategy_hash: row.strategy_hash, example_id: row.example_id });
        }
      }

      if (strategyScores.size === 0) {
        logger.info('[InputRouter] No strategies with valid scores after processing results.');
        return null;
      }
      ...
      strategyScores.forEach((agg, hash) => {
        ...
        logger.debug(`[InputRouter] Strategy ${hash}: Avg Score=${avgScore.toFixed(2)}, Successes=${agg.successCount}/${agg.count}, Avg Latency=${(agg.totalLatency/agg.count).toFixed(0)}ms, Avg Tokens=${(agg.totalCostTokens/agg.count).toFixed(0)}`);
      ...
      if (bestStrategyHash) {
        logger.info(`[InputRouter] Best strategy selected: ${bestStrategyHash} with average score ${maxAvgScore.toFixed(2)} for inputClass "${inputClass}", llmModelId "${llmModelId}"`);
      } else {
        logger.info(`[InputRouter] No best strategy found after aggregation for inputClass "${inputClass}", llmModelId "${llmModelId}".`);
      }
      return bestStrategyHash;

    } catch (error) {
      logger.error(`[InputRouter] Error getting best strategy: ${error.message}`, { stack: error.stack });
      return null;
    }
  }
