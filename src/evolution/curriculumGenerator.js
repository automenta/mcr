...
class CurriculumGenerator {
    constructor(config = {}) {
        this.config = config;
        this.generatedCasesDir = config.generatedCasesDir || path.join(__dirname, '..', 'evalCases', 'generated');
    ...
    async identifyPoorPerformingCases(limit = 5) {
        logger.info('[CurriculumGenerator] Identifying poor-performing evaluation cases...');
        try {
            ...
            const rows = await new Promise((resolve, reject) => {
                ...
                db.all(query, [limit], (err, rows) => {
                    if (err) {
                        logger.error(`[CurriculumGenerator] Error querying poor-performing cases: ${err.message}`);
                        return reject(err);
                    }
                    resolve(rows);
                });
            ...
            if (!rows || rows.length === 0) {
                logger.info('[CurriculumGenerator] No poor-performing cases found based on current criteria.');
                return [];
            }
            ...
            const enrichedCases = rows.map(row => {
                ...
                if (fullCase) {
                    return {
                        ...
                        failureRateProlog: row.total_runs > 0 ? row.prolog_failures / row.total_runs : 0,
                        avgPrologScore: row.avg_prolog_score,
                        avgAnswerScore: row.avg_answer_score,
                        sampleCaseDetails: { // Provide the original case for context to the LLM
            ...
            logger.info(`[CurriculumGenerator] Identified ${enrichedCases.length} poor-performing cases for variation.`);
            return enrichedCases;

        } catch (error) {
            logger.error(`[CurriculumGenerator] Error identifying poor-performing cases: ${error.message}`, { stack: error.stack });
            return [];
        }
    }
    ...
    async generateCaseVariations(originalCase, numVariations = 2) {
        if (!originalCase || !originalCase.naturalLanguageInput) {
            logger.warn('[CurriculumGenerator] Original case data is insufficient for generating variations.');
            return [];
        }
        ...
        if (!generatePromptTemplate) {
            logger.error("[CurriculumGenerator] GENERATE_EVAL_CASES template not found in prompts.js. Cannot generate variations.");
            return [];
        }

        const domain = originalCase.tags ? originalCase.tags.find(t => t.startsWith('domain_')) || 'general' : 'general';
...
`;
        ...
        try {
            logger.info(`[CurriculumGenerator] Requesting LLM to generate ${numVariations} variations for case: ${originalCase.exampleId || originalCase.naturalLanguageInput}`);
            const llmResponse = await llmService.generate(systemPrompt, userPrompt);

            if (!llmResponse) {
                logger.warn('[CurriculumGenerator] LLM returned empty response for case variations.');
                return [];
            }
            ...
            try {
                ...
                if (!Array.isArray(generatedCasesArray)) {
                    logger.warn('[CurriculumGenerator] LLM response for variations was not a JSON array. Response:', llmResponse.substring(0, 200));
                    return [];
                }
            } catch (parseError) {
                logger.error(`[CurriculumGenerator] Failed to parse LLM response as JSON array: ${parseError.message}. Response: ${llmResponse.substring(0,500)}`);
                ...
                if (jsonMatch && jsonMatch[1]) {
                    try {
                        ...
                        if (!Array.isArray(generatedCasesArray)) {
                             logger.warn('[CurriculumGenerator] Extracted content from markdown was not a JSON array.');
                             return [];
                        }
                         logger.info("[CurriculumGenerator] Successfully parsed JSON from markdown block after initial failure.");
                    } catch (e) {
                        logger.error(`[CurriculumGenerator] Failed to parse JSON from markdown block: ${e.message}`);
                        return [];
                    }
                } else {
                    return [];
            ...
            for (const newCase of generatedCasesArray) {
                if (newCase && newCase.id && newCase.naturalLanguageInput && newCase.inputType && newCase.expectedProlog) {
                    // Ensure unique ID, perhaps by appending a hash or more robust suffix
                    newCase.id = `${originalCase.exampleId}_var_${crypto.randomBytes(3).toString('hex')}_${newCase.id}`.replace(/[^a-zA-Z0-9_.-]/g, '_').substring(0, 100);
                    newCase.tags = [...(originalCase.tags || []), 'generated', 'variation'];
                    newCase.notes = `Generated variation of ${originalCase.exampleId}. Original desc: ${originalCase.description}. ${newCase.notes || ''}`.substring(0, 255);
                    finalNewCases.push(newCase);
                } else {
                    logger.warn('[CurriculumGenerator] LLM generated an invalid case structure:', newCase);
                }
            }
            logger.info(`[CurriculumGenerator] LLM generated ${finalNewCases.length} valid case variations.`);
            return finalNewCases;

        } catch (error) {
            logger.error(`[CurriculumGenerator] LLM call for case variations failed: ${error.message}`, { stack: error.stack });
            return [];
        }
    }
     ...
     */
    async generate() {
        logger.info('[CurriculumGenerator] Starting curriculum generation cycle...');
        ...
        if (poorCases.length === 0) {
            logger.info('[CurriculumGenerator] No poor-performing cases identified to base variations on. For now, generation will stop.');
        ...
        for (const poorCase of poorCases) {
            const variations = await this.generateCaseVariations(poorCase.sampleCaseDetails, 1); // Generate 1 variation per poor case for now
            if (variations.length > 0) {
                allNewlyGeneratedCases.push(...variations);
                // Persist these new cases to files
                this.saveGeneratedCases(variations, `variations_of_${poorCase.exampleId}`);
            }
        }

        if (allNewlyGeneratedCases.length > 0) {
            logger.info(`[CurriculumGenerator] Successfully generated ${allNewlyGeneratedCases.length} new evaluation cases in total.`);
        } else {
            logger.info('[CurriculumGenerator] No new evaluation cases were generated in this cycle.');
        }
    ...
    saveGeneratedCases(casesArray, baseName = 'generated_curriculum') {
        ...
        try {
            ...
            logger.info(`[CurriculumGenerator] Saved ${casesArray.length} generated cases to: ${filePath}`);
        } catch (error) {
            logger.error(`[CurriculumGenerator] Error saving generated cases to ${filePath}: ${error.message}`);
        }
    }
}
