...
class OptimizationCoordinator {
    ...
    async bootstrap() {
        ...
        try {
            ...
            logger.info('[Optimizer] Running evaluator to establish baseline performance data for all strategies...');
            await this.evaluator.run(); // evaluator.run() already handles DB init/close and evaluation
            logger.info('[Optimizer] Bootstrap phase completed. Performance database should be populated.');
        } catch (error) {
            logger.error(`[Optimizer] Error during bootstrap: ${error.message}`, { stack: error.stack });
        ...
        try {
            ...
            const rows = await new Promise((resolve, reject) => {
                ...
                db.all(query, [], (err, rows) => {
                    if (err) {
                        logger.error(`[Optimizer] Error querying performance_results for strategy selection: ${err.message}`);
                        return reject(err);
                    }
                    resolve(rows);
                });
            ...
            if (rows && rows.length > 0) {
                ...
                logger.info(`[Optimizer] Selected strategy hash for evolution: ${selectedStrategyHash} (avg exactMatchProlog: ${rows[0].avg_exactMatchProlog})`);
                ...
                if (foundStrategy) {
                    logger.info(`[Optimizer] Found strategy definition for hash ${selectedStrategyHash}: ID ${foundStrategy.id}`);
                    return foundStrategy;
                } else {
                    logger.warn(`[Optimizer] Could not find strategy definition for selected hash ${selectedStrategyHash}. This may happen if the strategy was dynamically generated and not in the initial set, or if hashing methods differ.`);
                    return null;
                }

            } else {
                logger.warn('[Optimizer] No suitable strategy found for evolution based on current criteria.');
                return null;
            }
        } catch (error) {
            logger.error(`[Optimizer] Error selecting strategy: ${error.message}`, { stack: error.stack });
    ...
    async evolveStrategy(strategyJson) {
        if (!strategyJson) {
            logger.warn("[Optimizer] No strategy JSON provided to evolve. Skipping evolution step.");
            return null;
        }
        ...
        logger.warn('[Optimizer] StrategyEvolver not yet implemented. Returning null.');
        return null;
    }
    ...
    async generateNewCurriculum(currentStrategy = null, failingCases = null) {
        ...
        // return newCases;
        logger.warn('[Optimizer] CurriculumGenerator not yet implemented. Returning empty array.');
        return [];
    }

    async evaluateStrategy(strategyJson, curriculumCases = null) {
        if (!strategyJson) {
            logger.warn("[Optimizer] No strategy JSON provided to evaluate. Skipping evaluation.");
            return;
        }
        ...
        const tempStrategyPath = path.join(tempStrategyDir, `${strategyJson.id}.json`);
        fs.writeFileSync(tempStrategyPath, JSON.stringify(strategyJson, null, 2));
        logger.info(`[Optimizer] Temporarily saved evolved strategy to ${tempStrategyPath}`);
        ...
        const singleStrategyEvaluator = new Evaluator(this.config.evalCasesPath || 'src/evalCases', [strategyJson.id]);
        try {
            logger.info(`[Optimizer] Running evaluator for new strategy ${strategyJson.id}...`);
            await singleStrategyEvaluator.run();
            logger.info(`[Optimizer] Evaluation of ${strategyJson.id} completed.`);
        } catch (error) {
            logger.error(`[Optimizer] Error evaluating strategy ${strategyJson.id}: ${error.message}`, { stack: error.stack });
        } finally {
            // Clean up the temporary strategy file
            try {
                fs.unlinkSync(tempStrategyPath);
                logger.info(`[Optimizer] Cleaned up temporary strategy file: ${tempStrategyPath}`);
                // Potentially remove the generated directory if empty, or leave for inspection.
            } catch (cleanupError) {
                logger.warn(`[Optimizer] Could not clean up temporary strategy file ${tempStrategyPath}: ${cleanupError.message}`);
            }
    ...
    async runIteration() {
        ...
        if (!strategyToEvolve) {
            logger.warn('[Optimizer] No strategy selected for evolution. Ending iteration.');
        ...
        if (!evolvedStrategy) {
            logger.warn('[Optimizer] Strategy evolution did not produce a new strategy. Ending iteration.');
    ...
    async start(iterations = 1) {
        logger.info(`[Optimizer] Starting optimization process for ${iterations} iteration(s).`);
        await initDb(); // Ensure DB is up for the whole process

        if (this.config.bootstrapOnly) {
            await this.bootstrap();
            logger.info('[Optimizer] BootstrapOnly flag set. Exiting after bootstrap.');
            await closeDb();
            return;
        }

        if (this.config.runBootstrap) { // Separate flag for explicit bootstrap before iterations
            await this.bootstrap();
        }
        ...
        for (let i = 0; i < iterations; i++) {
            ...
            if (!continueLoop) {
                logger.info('[Optimizer] Optimization loop conditions not met to continue. Stopping.');
                break;
            }
        }
...
async function main() {
    const argv = yargs(hideBin(process.argv))
        ...
        .option('runBootstrap', {
            type: 'boolean',
            description: 'Run bootstrap before starting iterations (if not bootstrapOnly).',
    ...
    logger.info(`[OptimizerCLI] Config: Iterations=${argv.iterations}, BootstrapOnly=${argv.bootstrapOnly}, RunBootstrap=${argv.runBootstrap}, EvalCasesPath=${argv.evalCasesPath}`);
    ...
    try {
        ...
        logger.info('[OptimizerCLI] Optimization Coordinator finished successfully.');
    } catch (error) {
        logger.error(`[OptimizerCLI] Critical error in Optimization Coordinator: ${error.message}`, { stack: error.stack });
        process.exit(1);
    }
}
