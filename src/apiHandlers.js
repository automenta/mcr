  ...
  try {
    ...
    if (result.success) {
      ...
      if (result.message === 'Session not found.') {
        ...
        result.error === 'no_facts_extracted'
      ) {
...
async function nlToRulesDirectHandler(req, res, next) {
  ...
  try {
    ...
    if (result.success) {
      ...
      res
        .status(200)
        .json({ rules: result.rules, rawOutput: result.rawOutput });
    } else {
      next(
        new ApiError(
    ...
    logger.error(
      `[API] Error in nlToRulesDirectHandler: ${error.message}`,
      error
    );
...
async function debugFormatPromptHandler(req, res, next) {
  ...
  try {
    ...
    logger.error(
      `[API] Error in debugFormatPromptHandler for ${templateName}: ${error.message}`,
      error
    );
...
async function explainQueryHandler(req, res, next) {
  ...
  try {
    logger.info(
      `[API] Explaining query for session ${sessionId}: "${naturalLanguageQuestion}"`
    );
    ...
    if (result.success) {
      ...
      if (
        result.error === 'invalid_prolog_query_explain' ||
    ...
    logger.error(
      `[API] Error in explainQueryHandler for session ${sessionId}: ${error.message}`,
      error
    );
...
async function rulesToNlDirectHandler(req, res, next) {
  ...
  try {
    logger.info(
      `[API] Translating Rules to NL (Direct). Style: ${style || 'conversational'}`
    );
    ...
    logger.error(
      `[API] Error in rulesToNlDirectHandler: ${error.message}`,
      error
    );
    next(new ApiError(500, `Translation failed: ${error.message}`));
  }
