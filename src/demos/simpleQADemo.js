...
async function runSimpleQADemoAsync() {
  ...
  demoLogger.info('Goal', 'Demonstrate basic session creation, fact assertion, and querying using mcrService.');
  let sessionId;
  ...
  try {
    ...
    if (!assertResult.success) {
      demoLogger.error('Failed to assert Fact 1', assertResult.message || assertResult.error);
      throw new Error(assertResult.message || assertResult.error || 'Assertion failed');
    }
    ...
    if (!assertResult.success) {
      demoLogger.error('Failed to assert Fact 2', assertResult.message || assertResult.error);
      throw new Error(assertResult.message || assertResult.error || 'Assertion failed');
    }
    demoLogger.logic('Added Facts (Prolog)', assertResult.addedFacts);
    ...
    const query1_response = await mcrService.querySessionWithNL(sessionId, query1_nl, { debug: true });
    if (!query1_response.success) {
        demoLogger.error('Query 1 failed', query1_response.message || query1_response.error);
        throw new Error(query1_response.message || query1_response.error || 'Query 1 failed');
    }
    demoLogger.logic('Generated Prolog Query', query1_response.debugInfo?.prologQuery);
    demoLogger.logic('Reasoner Result (Simplified)', query1_response.debugInfo?.prologResults); // This was simpleResult before
    ...
    if (query1_response.debugInfo) demoLogger.info('Debug Info', query1_response.debugInfo);
    ...
    const query2_response = await mcrService.querySessionWithNL(sessionId, query2_nl); // No debug option
    if (!query2_response.success) {
        demoLogger.error('Query 2 failed', query2_response.message || query2_response.error);
        throw new Error(query2_response.message || query2_response.error || 'Query 2 failed');
    }
    demoLogger.logic('Generated Prolog Query', query2_response.debugInfo?.prologQuery);
    demoLogger.logic('Reasoner Result (Simplified)', query2_response.debugInfo?.prologResults);
    ...
    if (err.stack && !err.message?.includes('Assertion failed') && !err.message?.includes('Query')) { // Avoid double printing stack for known errors
    ...
    if (sessionId) {
      ...
      try {
        ...
        demoLogger.cleanup(deleteResponse.message || `Session ${sessionId} deleted.`);
      } catch (cleanupError) {
