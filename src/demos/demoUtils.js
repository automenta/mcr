...
const demoLogger = {
  ...
  logic: (label, text) => console.log(`   🧠 \x1b[35m${label}:\x1b[0m ${typeof text === 'object' ? JSON.stringify(text) : text}`), // Magenta Logic
  ...
  error: (text, details) => {
    ...
    if (details) {
      ...
      console.error(`      \x1b[90mDetails: ${detailString.substring(0, maxDetailLength)}${detailString.length > maxDetailLength ? '...' : ''}\x1b[0m`);
    }
  },
...
const readFileContentSafe = (filePath, fileDescription = 'File') => {
  try {
    ...
    if (!fs.existsSync(resolvedPath)) {
      demoLogger.error(`${fileDescription} not found: ${resolvedPath}`,
        'Please ensure the file path is correct and the file exists at that location.');
      return null;
    }
    return fs.readFileSync(resolvedPath, 'utf8');
  } catch (error) {
    demoLogger.error(`Error reading ${fileDescription} "${filePath}"`, error.message);
    return null;
  }
};
