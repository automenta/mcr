...
async function runFamilyOntologyDemoAsync() {
  ...
  demoLogger.info(
    'Goal',
    'Demonstrate using a predefined ontology for complex queries and asserting additional facts.'
  );
  let sessionId;
  ...
  try {
    ...
    // 1. Load Ontology
    demoLogger.step(
      `1. Loading '${ontologyName}' ontology from '${ontologyFilePath}'`
    );
    const ontologyRules = readFileContentSafe(
      ontologyFilePath,
      'Family Ontology File'
    );
    if (!ontologyRules) {
      demoLogger.error(
        `Demo cannot proceed without ontology file: ${ontologyFilePath}.`
      );
      return;
    }
    ...
    try {
      ...
      demoLogger.cleanup(
        `Attempted to delete pre-existing ontology '${ontologyName}', if it existed.`
      );
    } catch (e) {
      if (e instanceof ApiError && e.statusCode === 404) {
        demoLogger.info(
          `Info: Pre-existing ontology '${ontologyName}' not found. No deletion needed.`
        );
      } else {
        demoLogger.error(
          `Warning: Could not delete pre-existing ontology '${ontologyName}'. Proceeding with loading.`,
          e.message
        );
    ...
    const loadedOntology = await ontologyService.createOntology(
      ontologyName,
      ontologyRules
    );
    ...
    const family_facts_nl =
      'Arthur is the father of William. Guinevere is the mother of William. William is the father of Lancelot. Igraine is the mother of Arthur.';
    ...
    let query_response = await mcrService.querySessionWithNL(
      sessionId,
      query_nl,
      { debug: true, dynamicOntology: ontologyRules }
    );
    if (!query_response.success) {
      demoLogger.error(
        "Query for William's father failed",
        query_response.message || query_response.error
      );
      throw new Error(
    ...
    demoLogger.logic(
      ...
      query_response.debugInfo?.prologQuery
    );
    demoLogger.logic(
      'Reasoner Result (Simplified)',
      query_response.debugInfo?.prologResults
    );
    demoLogger.mcrResponse('MCR Answer (NL)', query_response.answer);
    if (query_response.debugInfo)
      demoLogger.info('Debug Info', query_response.debugInfo);
    ...
    demoLogger.nl('Query (NL)', query_nl);
    query_response = await mcrService.querySessionWithNL(sessionId, query_nl, {
      debug: true,
      dynamicOntology: ontologyRules,
    });
    if (!query_response.success) {
      demoLogger.error(
        "Query for Lancelot's grandfather failed",
        query_response.message || query_response.error
      );
      throw new Error(
        query_response.message ||
    ...
    if (!query_response.success) {
      demoLogger.error(
        "Query for Arthur's mother failed",
        query_response.message || query_response.error
      );
      throw new Error(
        query_response.message ||
    ...
    if (err.stack && !err.message?.includes('failed')) {
      // Avoid double stack for known errors
    ...
    if (sessionId) {
      ...
      try {
        ...
        demoLogger.cleanup(
          deleteSessResp.message || `Session ${sessionId} deleted.`
        );
      } catch (cleanupError) {
        demoLogger.error(
    ...
    try {
      ...
      if (cleanupError instanceof ApiError && cleanupError.statusCode === 404) {
        demoLogger.info(
          `Info: Ontology '${ontologyName}' not found during cleanup. No deletion needed.`
        );
      } else {
        demoLogger.error(
          `Failed to delete ontology '${ontologyName}' during cleanup`,
          cleanupError.message
        );
        if (cleanupError.stack && !(cleanupError instanceof ApiError))
