...
**3. Start the MCR Server:**
   ...
   # OR using the CLI:
   # ./cli.js start-server
   # If you've linked cli.js globally (e.g. npm link), you might use: mcr-cli start-server
   ```
...
- **Error Responses**: Errors are returned in a consistent JSON format.
    ...
    // Example Error Response
    {
      "error": {
        ...
        "details": { /* Optional: additional structured details about the error */ }
      }
    }
    ```
...
- `POST /api/v1/sessions/:sessionId/assert`
  ...
  - **Response (200 OK)**:
    ...
    ```
    *(Note: Exact response structure for `addedFacts`, `message` may vary slightly based on `mcrService` output)*
...
- `POST /api/v1/sessions/:sessionId/query`
  ...
  - **Response (200 OK)**:
    ```json
    {
      ...
      "debugInfo": { // Included if options.debug was true and info is available
...
- `POST /api/v1/sessions/:sessionId/explain-query`
  ...
  - **Response (200 OK)**:
    ```json
    {
      ...
      "debugInfo": { /* Optional: relevant facts/rules considered for the explanation */ }
    }
    ```
...
- `POST /api/v1/translate/nl-to-rules`
  ...
  - **Response (200 OK)**:
    ```json
    {
      "rules": [ // Array of Prolog rule strings
        "can_fly(X) :- bird(X).",
        "bird(penguin).",
        "not(can_fly(penguin))."
      ],
...
2.  Add or update the `mcpServers` object:
    ...
    {
      "mcpServers": {
        "mcr-local-server": { // Choose any unique key
          "type": "url",
    ...
    - Example:
      ...
      const MyNewLlmProvider = {
        ...
        async generateStructured(systemPrompt, userPrompt, options = {}) {
          ...
          logger.debug(`MyNewLlmProvider generating text with model: ${model}`, { systemPrompt, userPrompt, options });
...
---
*Note: This README is based on analysis of the current project structure and information from `old/README.md` and `old/CODE.md`. Some details, especially example responses, might need further refinement based on live testing.*
```
