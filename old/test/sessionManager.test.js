// --- TOP OF FILE ---
const MOCK_SESSION_STORAGE_PATH_CONST =
  '/mocked_storage/sessions_data_sm_test_for_factory';
const MOCK_ONTOLOGY_STORAGE_PATH_CONST =
...
jest.mock('../src/errors', () => {
  ...
  return jest
    ...
    .mockImplementation(
      (status, message, code) =>
        new ActualApiErrorInsideFactory(status, message, code)
    );
});
...
describe('SessionManager', () => {
  ...
  beforeEach(() => {
    ...
    mockPathJoin
      .mockReset()
      .mockImplementation((...args) => args.filter(Boolean).join('/'));
  ...
  describe('Module Initialization', () => {
    test('should create ontology directory on load and session directory on first use if they do not exist', () => {
      ...
      mockFsExistsSync.mockImplementation(
        (p) =>
          !(p === MOCK_SESSION_STORAGE_PATH || p === MOCK_ONTOLOGY_STORAGE_PATH)
      );
      ...
      expect(mockFsMkdirSync).toHaveBeenCalledWith(MOCK_ONTOLOGY_STORAGE_PATH, {
        recursive: true,
      });
      ...
      expect(mockFsMkdirSync).not.toHaveBeenCalledWith(
        MOCK_SESSION_STORAGE_PATH,
        { recursive: true }
      );
      ...
      expect(mockFsMkdirSync).toHaveBeenCalledWith(MOCK_SESSION_STORAGE_PATH, {
        recursive: true,
      });
      expect(mockLoggerInfo).toHaveBeenCalledWith(
    ...
    test('should load all ontologies on initialization', () => {
      ...
      mockFsReadFileSync.mockImplementation((filePath) => {
        if (filePath === `${MOCK_ONTOLOGY_STORAGE_PATH}/family.pl`)
          return 'parent(X,Y).';
        if (filePath === `${MOCK_ONTOLOGY_STORAGE_PATH}/another.pl`)
          return 'rule(A,B).';
        return '';
      });
      mockLoggerInfo.mockClear();
      ...
      expect(mockFsReaddirSync).toHaveBeenCalledWith(
        MOCK_ONTOLOGY_STORAGE_PATH
      );
      ...
      expect(CurrentTestSessionManager._ontologies).toEqual({
        family: 'parent(X,Y).',
        another: 'rule(A,B).',
      });
      expect(mockLoggerInfo).toHaveBeenCalledWith(
        expect.stringContaining(
          `Loaded ${Object.keys(CurrentTestSessionManager._ontologies).length} ontologies`
        )
      );
    });
  });
  ...
  describe('Session Management', () => {
    ...
    test('get should load a session from file if not in memory', () => {
      ...
      mockFsExistsSync.mockImplementation(
        (p) => p === `${MOCK_SESSION_STORAGE_PATH}/file-id.json`
      );
  ...
  describe('Ontology Management', () => {
    ...
    test('getOntology should throw ApiError if ontology not found', () => {
      ...
      mockFsExistsSync.mockImplementation(
        (p) => p !== `${MOCK_ONTOLOGY_STORAGE_PATH}/non_existent_onto.pl`
      );
