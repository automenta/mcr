...
describe('ReasonerService', () => {
  ...
  describe('runQuery', () => {
    ...
    it('should execute a simple query and return a single result', async () => {
      ...
      mockSession.query.mockImplementationOnce((query, options) =>
        options.success()
      );
      mockSession.answer
        .mockImplementationOnce((callback) =>
          callback({ indicator: 'test/1', id: 'test', args: ['meow'] })
        )
        .mockImplementationOnce((callback) =>
    ...
    it('should return ["true."] for a query that succeeds without variables', async () => {
      ...
      mockSession.answer
        .mockImplementationOnce((callback) =>
          callback({ id: 'true', args: [], indicator: 'true/0' })
        )
        .mockImplementationOnce((callback) =>
          callback({ indicator: 'the_end/0' })
        );
    ...
    it('should return ["false."] for a query that fails explicitly (e.g. \\+ goal)', async () => {
      ...
      mockSession.answer
        .mockImplementationOnce((callback) =>
          callback({ id: 'false', args: [], indicator: 'fail/0' })
        ) // Assuming fail/0 maps to 'false' atom
        .mockImplementationOnce((callback) =>
          callback({ indicator: 'the_end/0' })
        );
    ...
    it('should return an empty array for a query with no solutions', async () => {
      ...
      mockSession.answer.mockImplementationOnce((callback) =>
        callback({ indicator: 'the_end/0' })
      );
      ...
      mockSession.answer
        .mockImplementationOnce((callback) =>
          callback({
            indicator: 'owns/2',
            id: 'owns',
            args: ['john', { functor: 'book', args: ['ulysses'] }],
          })
        )
        .mockImplementationOnce((callback) =>
          callback({ indicator: 'the_end/0' })
        );
      mockSession.format_answer.mockReturnValueOnce(
        'Person = john, Item = book(ulysses)'
      );
    ...
    it('should correctly process facts provided as a single string with multiple sentences', async () => {
      ...
      mockSession.answer
        .mockImplementationOnce((callback) =>
          callback({ indicator: 'flies/1', id: 'flies', args: ['robin'] })
        )
        .mockImplementationOnce((callback) =>
          callback({ indicator: 'the_end/0' })
        );
    ...
    describe('Directive Handling', () => {
      ...
      it('should correctly process facts including operator directives', async () => {
        ...
        const mockSession2 = {
          ...
          query: jest.fn(),
          answer: jest.fn(),
          format_answer: jest.fn().mockReturnValue('true.'),
        };
        mockSession2.consult.mockImplementationOnce((program, options) =>
          options.success()
        );
        mockSession2.query.mockImplementationOnce((query, options) =>
          options.success()
        );
        mockSession2.answer
          .mockImplementationOnce((callback) =>
        ...
        const results2 = await ReasonerService.runQuery(
          factsWithStandardRule,
          query2
        );
        ...
        mockSession3.consult.mockImplementationOnce((program, options) =>
          options.success()
        );
        mockSession3.query.mockImplementationOnce((query, options) =>
          options.success()
        );
        mockSession3.answer
          .mockImplementationOnce((callback) =>
            callback({
              indicator: 'rule/1',
              id: 'rule',
              args: ['(mortal(socrates) implies human(socrates))'],
            })
          ) // Simplified
          .mockImplementationOnce((callback) =>
            callback({ indicator: 'the_end/0' })
          );
        mockSession3.format_answer.mockReturnValueOnce(
          'R = (mortal(socrates) implies human(socrates))'
        );
        ...
        const resultsOpSyntax = await ReasonerService.runQuery(
          factsForOpSyntax,
          queryOpSyntax
        );
        expect(resultsOpSyntax).toContain(
          'R = (mortal(socrates) implies human(socrates))'
        );
      });

      it('should handle multiple directives correctly', async () => {
        const mockSessionFather = {
          consult: jest.fn(),
          query: jest.fn(),
          answer: jest.fn(),
          format_answer: jest.fn(),
        };
        mockSessionFather.consult.mockImplementationOnce((program, options) =>
          options.success()
        );
        mockSessionFather.query.mockImplementationOnce((query, options) =>
          options.success()
        );
        mockSessionFather.answer
          .mockImplementationOnce((callback) =>
            callback({ indicator: 'is_father_of/2', args: ['john', 'peter'] })
          )
          .mockImplementationOnce((callback) =>
            callback({ indicator: 'the_end/0' })
          );
        mockSessionFather.format_answer.mockReturnValueOnce(
          'X = john, Y = peter'
        );
        ...
        const queryForFather = 'X is_father_of Y.';
        const results = await ReasonerService.runQuery(facts, queryForFather);
        expect(results).toEqual(['X = john, Y = peter']);

        const mockSessionMother = {
          consult: jest.fn(),
          query: jest.fn(),
          answer: jest.fn(),
          format_answer: jest.fn(),
        };
        mockSessionMother.consult.mockImplementationOnce((program, options) =>
          options.success()
        );
        mockSessionMother.query.mockImplementationOnce((query, options) =>
          options.success()
        );
        mockSessionMother.answer
          .mockImplementationOnce((callback) =>
            callback({ indicator: 'is_mother_of/2', args: ['jane', 'peter'] })
          )
          .mockImplementationOnce((callback) =>
            callback({ indicator: 'the_end/0' })
          );
        mockSessionMother.format_answer.mockReturnValueOnce(
          'X = jane, Y = peter'
        );
        pl.create.mockReturnValueOnce(mockSessionMother);
        const queryForMother = 'X is_mother_of Y.';
        const results2 = await ReasonerService.runQuery(facts, queryForMother);
